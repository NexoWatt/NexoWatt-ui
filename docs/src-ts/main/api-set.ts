...
export type MainApiValueKind = 'boolean' | 'number' | 'string' | 'json';

export interface MainApiSetRequest<T = unknown> {
  readonly scope: string;
  readonly key: string;
  readonly value: T;
  readonly source?: 'frontend' | 'admin' | 'automation' | 'unknown';
}

export interface MainSettingDefinition {
  readonly key: string;
  readonly stateId: string;
  readonly kind: MainApiValueKind;
  readonly min?: number;
  readonly max?: number;
}

export interface MainStateWritePlan<T = unknown> {
  readonly stateId: string;
  readonly value: T;
...
export interface MainApiSetWriteResult<T = unknown> {
  readonly ok: boolean;
  readonly message?: string;
  readonly plan?: MainStateWritePlan<T>;
}

export const DEFAULT_MAIN_SETTING_DEFINITIONS: readonly MainSettingDefinition[] = [
...
function normalizeMainBoolean(value: unknown): boolean {
...
function normalizeMainNumber(value: unknown, min?: number, max?: number): number {
  let n = Number(value);
 ...
 */
export function normalizeMainApiSetValue(value: unknown, kind: MainApiValueKind, definition?: Pick<MainSettingDefinition, 'min' | 'max'>): unknown {
  if (kind === 'boolean') return normalizeMainBoolean(value);
  if (kind === 'number') return normalizeMainNumber(value, definition?.min, definition?.max);
  if (kind === 'json') {
    if (typeof value === 'string') {
      try { return JSON.parse(value) as unknown; } catch (_err) { return null; }
    }
    return value ?? null;
  }
  return String(value ?? '');
 ...
 */
export function buildMainSettingsWritePlan(request: MainApiSetRequest, definitions: readonly MainSettingDefinition[] = DEFAULT_MAIN_SETTING_DEFINITIONS): MainApiSetWriteResult {
...
export interface MainApiSetShadowComparison<T = unknown> {
 ...
 */
export function buildMainApiSetShadowComparison(request: MainApiSetRequest, definitions: readonly MainSettingDefinition[] = DEFAULT_MAIN_SETTING_DEFINITIONS): MainApiSetShadowComparison {

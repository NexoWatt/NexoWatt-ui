 ...
 * Quell-Hash: sha256:9f3c3dc7a95f768b8230cfab39f0639ebb96c4782572b1c52e991d904988c4d3
...
function technicalMinPowerW(wb) {
    if (wb.minPowerW > 0)
        return wb.minPowerW;
    if (wb.minA > 0)
        return Math.round(Math.max(1, wb.phases || 1) * Math.max(1, wb.voltageV || 230) * wb.minA);
    return 0;
}
function technicalMaxPowerW(wb) {
    if (wb.maxPowerW > 0)
        return wb.maxPowerW;
    if (wb.maxA > 0)
        return Math.round(Math.max(1, wb.phases || 1) * Math.max(1, wb.voltageV || 230) * wb.maxA);
    return wb.chargerType === 'dc' ? 50000 : 11000;
}
function currentForPowerA(wb, targetPowerW) {
    const denom = Math.max(1, wb.phases || 1) * Math.max(1, wb.voltageV || 230);
    let amps = targetPowerW > 0 ? targetPowerW / denom : 0;
    if (amps > 0 && wb.minA > 0 && amps < wb.minA)
        amps = wb.minA;
    if (wb.maxA > 0 && amps > wb.maxA)
        amps = wb.maxA;
...
function isChargingCandidate(wb) {
    ...
    if (!wb.hasSetpoint)
        return false;
    if (modeText.includes('off') || modeText.includes('disabled') || modeText.includes('aus'))
        return false;
    ...
    if (storageRemainingW > 0) {
        ...
        while (storageRemainingW > 0 && guardStorage < 20) {
            ...
            for (const item of adjustable) {
                ...
                usedThisRound += add;
                if (usedThisRound >= storageRemainingW)
                    break;
            }
            if (usedThisRound <= 0)
                break;

...
- Korrektur des Windows-Publish-Hotfixes: `npm ci` installiert gemäß Lockfile TypeScript 6.0.3, während die zuvor ausgelieferten Runtime-Artefakte in einer abweichenden Compiler-Umgebung erzeugt worden waren. Dadurch meldete `check:ts-runtime-executables` ausschließlich `www/energy-origin-appcenter.js` und `www/energy-origin-ledger-view.js` als veraltet. `prepublishOnly` synchronisiert die Runtime-Dateien nun nach der Versionsprüfung automatisch mit dem lokal installierten, exakt auf 6.0.3 gepinnten Compiler, aktualisiert anschließend die davon abhängigen Runtime-TS-Spiegel und startet erst danach das vollständige Publish-Gate. Ein frisches `npm ci` kann damit keinen reproduzierbaren Compiler-/Artefakt-Widerspruch mehr erzeugen.
- Der statische Publish-Guard prüft jetzt zusätzlich die exakte Reihenfolge Versionsschutz → Runtime-Sync → Spiegel-Sync → Gesamtprüfung sowie die identische TypeScript-6.0.3-Festlegung in `package.json` und `package-lock.json`.

...
- Hartes Anti-Export-Sicherheitsgate für Speicher und Speicherfarm ergänzt. Jede positive Entladevorgabe wird nach Tarif-, MultiUse-, EVCS-, Budget- und Herstellerlogik, aber unmittelbar vor 0-W-Firewall und Hardwarewriter nochmals gegen den frischen signierten NVP geprüft. Bestätigte Einspeisung beendet die Entladung mit einem ausdrücklichen 0-W-Stopp; Sungrow-/FENECON-Hold- und No-Write-Pfade dürfen einen alten Entladebefehl dabei nicht weiterführen.
- Entladeleistung wird präventiv auf den physikalisch sicheren NVP-Headroom begrenzt: `max(0, Speicher-Ist + NVP-Ist - NVP-Ziel)`. Damit kann die Eigenverbrauchsregelung den kleinen Zielbezug anfahren, aber ein Feed-forward-, Tarif-, MultiUse- oder gehaltener Herstellerbefehl den NVP nicht absichtlich in die Einspeisung drücken.
- Sicherheitsorientierter Messvertrag: Eine positive Entladung wird ohne frischen signierten NVP oder ohne frische, vertrauenswürdige physische Speicher-Istleistung fail-safe auf 0 W gestoppt. Gehaltene beziehungsweise prognostizierte Async-Werte reichen für diese letzte Sicherheitsgrenze nicht aus; für automatisches Entladen muss der manuell zugeordnete Istleistungs-DP daher aktuell und plausibel sein. AppCenter → Speicher kennzeichnet diesen Messwert nun ausdrücklich als Voraussetzung für sichere Entladung; die Speicherfarm weist entsprechend auf einen erforderlichen frischen Signed- oder Split-Istleistungsweg hin.
- Sungrow-NVP-Ausfallpfad gehärtet: Bei fehlendem NVP darf nur eine bestehende Lade-/Idle-Vorgabe während der begrenzten Grace-Phase gehalten werden. Ein alter positiver Entladebefehl wird nicht mehr per `hold-last-active-command` oder `no-write` weitergeführt.
- LIVE/History physikalisch kohärent: gleichzeitig belegte Import-/Export-DPs sowie Lade-/Entlade-Istwerte werden zu einem exklusiven signierten Nettofluss normalisiert. Die Rohwerte und ein Konflikthinweis bleiben diagnostisch erhalten, die Oberfläche zeigt jedoch nicht mehr gleichzeitig Netzbezug und Netzeinspeisung beziehungsweise Laden und Entladen.
- AppCenter → Status bleibt unverändert kompakt. Anti-Export-Aktion, sicherer Entlade-Cap, prognostizierter NVP und vollständige Entscheidung stehen ausschließlich in den vorhandenen States `speicher.regelung.antiExport*`, `ems.nvpCoordinator.*` und im Stabilitätslog.
- Neue Regressionen `test:storage-anti-export-hard-gate`, `test:storage-anti-export-guard` und `test:history-flow-coherence` prüfen Nacht-Export, NVP-Headroom, fehlende Messwerte, Sungrow-Stopp/Readback sowie kohärente History-Flüsse.
- TypeScript-Migrationskontrolle verbessert: der kleine SG-Ready-Consumer wurde aus `@ts-nocheck` herausgeführt. Damit sinkt der offen ausgewiesene Bestand trotz der neuen Sicherheitsintegration von 58 auf 57 Dateien und von 144.090 auf 144.084 Zeilen. Service-Worker-Cache auf `nexowatt-cache-v439` erhöht.

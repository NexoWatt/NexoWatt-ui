...
/**
 * SmartHome-Konfigurationsvertrag.
 *
 * Zweck:
 * - migriert historische UI-Schemata auf einen gemeinsamen Runtime-Vertrag,
 * - hält Template-Fähigkeiten und Schreibverhalten eindeutig,
 * - validiert strukturelle Fehler vor dem Aktivieren einer Konfiguration.
 *
 * Sicherheitsprinzip:
 * Fehlende Datenpunkte sind zulässig, damit ein Gerät schrittweise eingerichtet
 * werden kann. Sie erzeugen aber niemals automatisch einen Schreibpfad. Die
 * Runtime stellt solche Geräte als "nicht eingerichtet/unbekannt" dar.
 */


type AnyRecord = Record<string, any>;

type WritableValueSpec = {
  valueType: string;
  min: number;
  max: number;
  step: number;
...
type SmartHomeDevice = AnyRecord & {
  id: string;
  alias: string;
  type: string;
  behavior: AnyRecord;
...
type SmartHomeSceneAction = AnyRecord & {
  id: string;
...
type SmartHomeScene = AnyRecord & {
  id: string;
  alias: string;
  actions: SmartHomeSceneAction[];
};

type SmartHomeConfig = AnyRecord & {
  version: number;
  floors: any[];
  rooms: any[];
  functions: any[];
...
const SUPPORTED_SCENE_ACTIONS: Set<string> = new Set([
...
const WRITABLE_VALUE_TEMPLATES: Record<string, WritableValueSpec> = {
...
function isPlain(value: unknown): value is AnyRecord {
...
function cleanIoPair(source: unknown, fallbackRead?: unknown, fallbackWrite?: unknown): { readId: string | null; writeId: string | null } {
...
function normalizeTemplateContract(templateId: unknown, type: string, behavior: AnyRecord, capabilities: AnyRecord, io: AnyRecord, ui: AnyRecord): TemplateContract {
  ...
  if (tid === 'button_press_release' || tid === 'iot_trigger') {
    ...
    if (!str(out.ui.unit, 32) && spec.unit) out.ui.unit = spec.unit;
  }

  return out;
}

function normalizeSmartHomeDevice(input: unknown, index = 0): SmartHomeDevice {
  ...
  if (!SUPPORTED_TYPES.has(type)) type = 'switch';

  const id = str(raw.id || `device_${index + 1}`, 80) || `device_${index + 1}`;
  const behavior: AnyRecord = {
  ...
  const capabilities: AnyRecord = { ...rawCapabilities };
  const ui: AnyRecord = {
    ...rawUi,
  ...
  if (sw.readId || sw.writeId) io.switch = sw;
  ...
  if (levelRead || levelWrite || type === 'dimmer' || type === 'blind' || type === 'color') {
    ...
    if (io.level.max < io.level.min) [io.level.min, io.level.max] = [io.level.max, io.level.min];
  }
  ...
  if (type === 'blind' || Object.keys(coverRaw).length || Object.keys(legacyBlind).length) {
    const positionReadId = nullableId(
      coverRaw.positionId
      || coverRaw.readId
      || legacyBlind.posId
      || (io.level && io.level.readId),
    );
    const positionWriteId = nullableId(
      coverRaw.positionWriteId
      || coverRaw.writeId
  ...
  if (type === 'sensor' || Object.keys(sensorRaw).length) {
    ...
    if (io.sensor.max < io.sensor.min) [io.sensor.min, io.sensor.max] = [io.sensor.max, io.sensor.min];
  }
  ...
  if (type === 'camera' || type === 'widget') contract.behavior.readOnly = true;
  if (type === 'scene' && contract.behavior.commandMode === 'toggle') contract.behavior.commandMode = 'momentary';
...
function normalizeSceneActionKind(value: unknown): string {
...
function normalizeSmartHomeScene(input: unknown, index = 0): SmartHomeScene {
  const raw = isPlain(input) ? input : {};
  ...
  return {
    ...
    actions: (Array.isArray(raw.actions) ? raw.actions : []).map((action: unknown, actionIndex: number): SmartHomeSceneAction => {
...
function sceneActionTargetAvailable(dev: SmartHomeDevice | null | undefined, kind: unknown, value: unknown): boolean {
  const io = dev && dev.io ? dev.io : {};
  const normalized = normalizeSceneActionKind(kind);
  if (normalized === 'switch' || normalized === 'on' || normalized === 'off') return !!(io.switch && io.switch.writeId);
  if (normalized === 'level') return !!(io.level && io.level.writeId);
  if (normalized === 'covertilt') return !!(io.cover && io.cover.tiltWriteId);
  if (normalized === 'cover') {
    ...
    if (typeof action === 'string' && ['up', 'down', 'stop'].includes(action)) {
      ...
      if (action === 'up') return !!(cover.upId || cover.actionId);
      if (action === 'down') return !!(cover.downId || cover.actionId);
      return !!(cover.stopId || cover.actionId);
    }
    return !!(io.level && io.level.writeId);
  }
  if (normalized === 'rtrsetpoint') return !!(io.climate && io.climate.setpointId);
  if (normalized === 'color') return !!(io.color && io.color.writeId);
  if (normalized === 'white') return !!(io.white && io.white.writeId);
  if (normalized === 'colortemperature') return !!(io.colorTemperature && io.colorTemperature.writeId);
  if (normalized === 'climatepower') return !!(io.climate && io.climate.powerId);
  if (normalized === 'climatemode') return !!(io.climate && io.climate.modeId);
  if (normalized === 'climatefan') return !!(io.climate && io.climate.fanSpeedId);
  if (normalized === 'climateswing') return !!(io.climate && io.climate.swingId);
  if (normalized === 'value') return !!(io.sensor && io.sensor.writeId);
  if (normalized === 'playervolume') return !!(io.player && io.player.volumeWriteId);
  if (normalized === 'playerseek') return !!(io.player && io.player.seekWriteId);
  if (normalized === 'playermute') return !!(io.player && io.player.muteWriteId);
  if (normalized === 'playerpower') return !!(io.player && io.player.powerWriteId);
  if (normalized === 'playertts') return !!(io.player && io.player.ttsWriteId);
  if (normalized === 'playershuffle') return !!(io.player && io.player.shuffleId);
  if (normalized === 'playerrepeat') return !!(io.player && io.player.repeatId);
  if (normalized === 'playerstation') return !!(io.player && io.player.stationId);
  if (normalized === 'playerplaylist') return !!(io.player && io.player.playlistId);
  if (normalized === 'player') {
    const action = str(value, 40).toLowerCase();
    const player = io.player || {};
    if (action === 'play') return !!(player.playId || (player.toggleId && player.playingId));
    if (action === 'pause') return !!(player.pauseId || (player.toggleId && player.playingId));
    if (action === 'stop') return !!player.stopId;
    if (action === 'next') return !!player.nextId;
    if (action === 'prev' || action === 'previous') return !!player.prevId;
    return false;
  }
  return false;
}

function normalizeSmartHomeConfig(input: unknown): SmartHomeConfig {
  const raw = isPlain(input) ? input : {};
  return {
    ...raw,
    ...
    devices: (Array.isArray(raw.devices) ? raw.devices : []).map((row: unknown, idx: number) => normalizeSmartHomeDevice(row, idx)),
    scenes: (Array.isArray(raw.scenes) ? raw.scenes : []).map((row: unknown, idx: number) => normalizeSmartHomeScene(row, idx)),
    pages: Array.isArray(raw.pages) ? raw.pages : [],
    meta: isPlain(raw.meta) ? raw.meta : {},
  };
}

function validateSmartHomeConfig(input: unknown): { ok: boolean; config: SmartHomeConfig; errors: ValidationIssue[]; warnings: ValidationIssue[] } {
  const config = normalizeSmartHomeConfig(input);
  const errors: ValidationIssue[] = [];
  const warnings: ValidationIssue[] = [];
  const seen: Set<string> = new Set();
  const push = (target: ValidationIssue[], code: string, message: string, path: string, deviceId?: string): void => {
    target.push(deviceId ? { code, message, path, deviceId } : { code, message, path });
  };

  config.devices.forEach((dev: SmartHomeDevice, index: number) => {
    const base = `devices[${index}]`;
    if (!dev.id) push(errors, 'DEVICE_ID_MISSING', 'Geräte-ID fehlt.', `${base}.id`);
    else if (seen.has(dev.id)) push(errors, 'DEVICE_ID_DUPLICATE', `Geräte-ID "${dev.id}" ist doppelt.`, `${base}.id`, dev.id);
    else seen.add(dev.id);
    if (!SUPPORTED_TYPES.has(dev.type)) push(errors, 'DEVICE_TYPE_INVALID', `Unbekannter Gerätetyp "${dev.type}".`, `${base}.type`, dev.id);
  ...
  const deviceById: Map<string, SmartHomeDevice> = new Map(config.devices.map((device: SmartHomeDevice) => [device.id, device]));
  const sceneById: Map<string, SmartHomeScene> = new Map();
  config.scenes.forEach((scene: SmartHomeScene, index: number) => {
    const base = `scenes[${index}]`;
    if (!scene.id) push(errors, 'SCENE_ID_MISSING', 'Szenen-ID fehlt.', `${base}.id`);
    else if (sceneById.has(scene.id)) push(errors, 'SCENE_ID_DUPLICATE', `Szenen-ID "${scene.id}" ist doppelt.`, `${base}.id`);
    else sceneById.set(scene.id, scene);
  ...
  config.scenes.forEach((scene: SmartHomeScene, sceneIndex: number) => {
    const actions = Array.isArray(scene.actions) ? scene.actions : [];
    actions.forEach((action: SmartHomeSceneAction, actionIndex: number) => {
      ...
      if (kind === 'scene') {
        ...
        if (!targetSceneId) push(errors, 'SCENE_TARGET_MISSING', 'Verschachtelte Szene benötigt eine Ziel-Szenen-ID.', `${base}.value`, scene.id);
        else if (!sceneById.has(targetSceneId)) push(errors, 'SCENE_TARGET_UNKNOWN', `Zielszene "${targetSceneId}" existiert nicht.`, `${base}.value`, scene.id);
        return;
      }
      const deviceId = str(action.deviceId, 80);
      const dev = deviceById.get(deviceId);
      if (!deviceId) push(errors, 'SCENE_DEVICE_MISSING', 'Szenenaktion benötigt ein Zielgerät.', `${base}.deviceId`, scene.id);
      else if (!dev) push(errors, 'SCENE_DEVICE_UNKNOWN', `Zielgerät "${deviceId}" existiert nicht.`, `${base}.deviceId`, scene.id);
      else if (dev.behavior && dev.behavior.readOnly) push(errors, 'SCENE_DEVICE_READONLY', `Zielgerät "${dev.alias || deviceId}" ist nur lesbar.`, `${base}.deviceId`, scene.id);
  ...
  const sceneAdjacency: Map<string, string[]> = new Map(Array.from(sceneById.keys()).map((id: string) => [id, [] as string[]]));
  for (const scene of config.scenes) {
    for (const action of Array.isArray(scene.actions) ? scene.actions : []) {
      if (normalizeSceneActionKind(action.kind) !== 'scene') continue;
      const target = str(action.value || action.sceneId, 80);
      if (target && sceneAdjacency.has(scene.id) && sceneAdjacency.has(target)) sceneAdjacency.get(scene.id)!.push(target);
    }
  }
  const sceneVisiting: Set<string> = new Set();
  const sceneVisited: Set<string> = new Set();
  const sceneStack: string[] = [];
  const visitScene = (id: string): void => {
    ...
    if (sceneVisited.has(id)) return;
    sceneVisiting.add(id);
    sceneStack.push(id);
    for (const next of sceneAdjacency.get(id) || []) visitScene(next);
    sceneStack.pop();
    sceneVisiting.delete(id);
    sceneVisited.add(id);
  };
  for (const id of sceneAdjacency.keys()) visitScene(id);

  return { ok: errors.length === 0, config, errors, warnings };
}

function collectIds(io: unknown, mode: 'read' | 'write'): string[] {
  const out: string[] = [];
  const readableKeys: Set<string> = new Set([
  ...
  const writableKeys: Set<string> = new Set([
  ...
  const visit = (value: unknown, key: string): void => {
    ...
    for (const [childKey, childValue] of Object.entries(value)) {
      if (childValue && typeof childValue === 'object') visit(childValue, childKey);
        ...
        if (matches) out.push(childValue.trim());
      }
    }
  };
  visit(io, '');

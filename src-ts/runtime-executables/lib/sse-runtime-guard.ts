...
class SseRuntimeGuard {
  ...
  _bind(client, emitter, event, handler) {
    ...
    const wrapped = (...args) => {
      try {
        ...
        if (index >= 0) client.listeners.splice(index, 1);
      } catch (_error) {}
      return handler(...args);
    };
    row.handler = wrapped;
  ...
  _detach(client) {
    for (const row of client?.listeners || []) {
      try { row.emitter?.removeListener?.(row.event, row.handler); } catch (_error) {}
    }
    if (client) client.listeners = [];
  }

  addClient(input = {}) {
    const req = input.req;
    const res = input.res;
    if (!req || !res || res.destroyed || res.writableEnded) return null;
    ...
    while (this.clients.size >= this.maxClients) {
      ...
      for (const candidate of this.clients) {
        if (!oldest || (candidate.connectedAt || 0) < (oldest.connectedAt || 0)) oldest = candidate;
      }
      if (!oldest) break;
    ...
    try { res?.setTimeout?.(0); } catch (_error) {}
    try { socket?.setKeepAlive?.(true, this.heartbeatMs); } catch (_error) {}
    try { socket?.setNoDelay?.(true); } catch (_error) {}
  ...
  _bindDrain(client) {
    if (!client || client.drainBound || this._isDead(client)) return;
  ...
  _resync(client) {
    if (!this.getSnapshotChunk || this._isDead(client)) return;
    try {
  ...
  write(client, chunk, meta = {}) {
    if (!client || !this.clients.has(client) || this._isDead(client)) {
      if (client) this.close(client, 'dead-before-write');
      return false;
    }
  ...
  close(client, reason = 'closed') {
    ...
    try { client.res?.end?.(); } catch (_error) {}
    try {
      const socket = client.res?.socket || client.req?.socket;
      if (socket && !socket.destroyed && /(?:backpressure|buffer|pressure|error|limit|unload)/.test(String(reason))) socket.destroy();
    } catch (_error) {}
    if (!this.clients.size) this._stopHeartbeat();
  }

  closeAll(reason = 'close-all') {
    for (const client of Array.from(this.clients)) this.close(client, reason);
    this.clients.clear();
    this._stopHeartbeat();
  }
  ...
  _ensureHeartbeat() {
    if (this._heartbeatTimer || !this.clients.size) return;
  ...
  getStats() {
    ...
    for (const client of this.clients) {
      if (client.backpressuredAt > 0) backpressured += 1;

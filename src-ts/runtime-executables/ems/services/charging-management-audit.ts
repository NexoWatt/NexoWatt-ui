...
/**
 * Read-only charging-management audit model and bounded event store.
 * This service never writes a charging actuator; it only publishes diagnostic states.
 */
declare const module: { exports: unknown };

type AnyRecord = Record<string, any>;
type QueueState = (id: string, value: unknown, ack: boolean) => Promise<unknown> | unknown;
type FlushQueue = () => Promise<unknown> | unknown;

function finiteChargingAuditNumber(value: unknown, fallback: any = null): any {
...
function compactChargingAuditText(value: unknown, maxLen = 240): string {
 ...
 */
function isChargingAuditInformationalLimiter(value: unknown): boolean {
...
function isChargingAuditActiveLimiter(value: unknown): boolean {
    return !isChargingAuditInformationalLimiter(value);
}

function deriveChargingAuditLimiter(entry: AnyRecord = {}, global: AnyRecord = {}): string {
    ...
    if (global.safetyStop === true || global.safetyValid === false || global.safetyEmergencyStop === true) return 'eos-safety-stop';
    if (entry.faultActive === true || reason.includes('FAULT')) return 'fault';
    if (entry.unavailableActive === true || reason.includes('UNAVAILABLE')) return 'unavailable';
    if (entry.online === false || reason.includes('OFFLINE')) return 'offline';
    if (entry.enabled === false || reason === 'DISABLED' || reason.includes('CONTROL_DISABLED')) return 'disabled';
    if (entry.meterStale === true || reason.includes('STALE_METER') || reason.includes('STALE-METER')) return 'stale-meter-failsafe';
    if (safetyBinding.includes('grid') && safetyBinding.includes('phase')) return 'grid-and-phase';
    if (safetyBinding.includes('grid')) return 'grid-import';
    if (safetyBinding.includes('phase')) return 'phase';
    if (safetyBinding.includes('14') || safetyBinding.includes('para')) return 'para14a';
    if (safetyBinding.includes('station')) return 'station';
    if (safetyBinding.includes('device') || safetyBinding.includes('user')) return 'device';
    if (reason.includes('LIMITED_BY_GRID_IMPORT')) return 'grid-import';
    if (reason.includes('LIMITED_BY_PHASE_CAP') || reason.includes('LIMIT_PHASE')) return 'phase';
    if (reason.includes('LIMITED_BY_14A') || reason.includes('PARA14A')) return 'para14a';
    if (reason.includes('LIMITED_BY_STATION_CAP')) return 'station';
    if (reason.includes('LIMITED_BY_USER_LIMIT') || reason.includes('SAFETY-CLAMPED:DEVICE')) return 'device';
    if (reason.includes('PAUSED_BY_PEAK_SHAVING') || String(global.status || '').includes('peak')) return 'peak-shaving';
    if (reason.includes('NO_PV_SURPLUS')) return 'pv-surplus';
    if (reason.includes('NO_BUDGET') || reason.includes('LIMITED_BY_BUDGET') || reason.includes('BELOW_MIN')) return 'budget';
    if (reason.includes('NO_VEHICLE')) return 'no-vehicle';
    if (entry.vehicleDemandConfirmed === false && entry.connected === true && Number(entry.targetW || 0) <= 0) return 'no-charge-demand';
    if (reason.includes('NO_SETPOINT')) return 'no-setpoint';
    if (applyStatus.includes('write_failed') || applyStatus.includes('executor_error') || applyStatus.includes('unreachable')) return 'write-error';
    return 'none';
}

function deriveChargingAuditGlobalLimiter(input: AnyRecord = {}, wallboxes: AnyRecord[] = []): string {
    const status = String(input.status || '').toLowerCase();
    if (input.safetyStop === true || input.safetyValid === false || input.safetyEmergencyStop === true) return 'eos-safety-stop';
    if (status.includes('failsafe') || status.includes('stale')) return 'stale-meter-failsafe';
    if (input.pausedByPeakShaving === true || status.includes('peak')) return 'peak-shaving';
    if (input.para14aBinding === true) return 'para14a';
    if (input.gridCapBinding === true && input.phaseCapBinding === true) return 'grid-and-phase';
    if (input.gridCapBinding === true) return 'grid-import';
    if (input.phaseCapBinding === true) return 'phase';
    const perLp = Array.isArray(wallboxes) ? wallboxes.map((row: AnyRecord) => String(row.limiter || 'none')) : [];
    for (const limiter of ['grid-and-phase', 'grid-import', 'phase', 'para14a', 'station', 'device', 'write-error', 'no-setpoint', 'budget', 'peak-shaving']) {
        if (perLp.includes(limiter)) return limiter;
    }
    return 'none';
}

function buildChargingAuditSnapshot(input: AnyRecord = {}): AnyRecord {
    ...
    const debugBySafe = new Map<string, AnyRecord>();
    for (const row of debugRows) {
        if (!row || typeof row !== 'object' || row.type === 'budget') continue;
        const safe = String(row.safe || '').trim();
        if (safe) debugBySafe.set(safe, row);
    }
    const runtimeBySafe = new Map<string, AnyRecord>();
    for (const row of runtimeRows) {
        if (!row || typeof row !== 'object') continue;
        const safe = String(row.safe || '').trim();
        if (safe) runtimeBySafe.set(safe, row);
    }
    ...
    const safetyStageByLimiter: Readonly<Record<string, string>> = Object.freeze({
    ...
    snapshot.problemCount = wallboxes.filter((row: AnyRecord) => !['none', 'no-vehicle', 'no-charge-demand', 'pv-surplus'].includes(row.limiter)).length
        + (snapshot.safety.valid === false || snapshot.safety.emergencyStop ? 1 : 0);
    return snapshot;
}

function chargingAuditEventSignature(snapshot: AnyRecord | null): string {
    if (!snapshot || typeof snapshot !== 'object') return '';
    const wallboxBits = (Array.isArray(snapshot.wallboxes) ? snapshot.wallboxes : []).map((row: AnyRecord) => [
        row.safe,
        row.mode,
        row.online ? 1 : 0,
        row.connected ? 1 : 0,
...
function buildChargingAuditEvents(previousSnapshot: AnyRecord | null, snapshot: AnyRecord, options: AnyRecord = {}): AnyRecord[] {
    if (!snapshot || typeof snapshot !== 'object') return [];
    const now = Math.max(0, Math.round(finiteChargingAuditNumber(snapshot.ts, Date.now())));
    const previousBySafe = new Map<string, AnyRecord>((previousSnapshot && Array.isArray(previousSnapshot.wallboxes) ? previousSnapshot.wallboxes : [])
        .filter((row: AnyRecord) => row && row.safe)
        .map((row: AnyRecord) => [String(row.safe), row]));
    ...
    for (const row of Array.isArray(snapshot.wallboxes) ? snapshot.wallboxes : []) {
        ...
        ].some((key: string) => previous[key] !== row[key]);
        if (!changed) continue;
        ...
        const deltas: string[] = [];
        if (previous) {
            if (previous.targetPowerW !== row.targetPowerW) deltas.push(`Soll ${previous.targetPowerW}→${row.targetPowerW} W`);
            if (previous.actualPowerW !== row.actualPowerW) deltas.push(`Ist ${previous.actualPowerW}→${row.actualPowerW} W`);
            if (previous.limiter !== row.limiter) deltas.push(`Limit ${previous.limiter}→${row.limiter}`);
            if (previous.applyStatus !== row.applyStatus) deltas.push(`Write ${previous.applyStatus || '—'}→${row.applyStatus || '—'}`);
        }
        events.push({
            ts: now,
            type: 'wallbox',
...
class ChargingManagementAuditStore {
  ...
  private events: AnyRecord[] = [];
  private snapshot: AnyRecord | null = null;
  private lastSignature = '';
  private lastEventMs = 0;
  private eventSeq = 0;
  private readonly maxEvents = 240;
  private readonly heartbeatMs = 60_000;

  constructor(adapter: AnyRecord, queueState: QueueState, flushQueue: FlushQueue) {
  ...
  async initialize(): Promise<void> {
    const ensureState = async (id: string, name: string, type: string, role: string): Promise<void> => {
    ...
    this.adapter._nwChargingManagementAudit = {
      schemaVersion: 1,
      getSnapshot: (): AnyRecord | null => this.getSnapshot(),
      getEvents: (limit: unknown): AnyRecord => this.getPayload(limit),
      clear: (): Promise<AnyRecord> => this.clear(),
    };
  }
  ...
  async restore(): Promise<void> {
    ...
    } catch { this.snapshot = null; this.lastSignature = ''; }
  ...
  getSnapshot(): AnyRecord | null { return this.snapshot; }

  getPayload(limit: unknown = 200): AnyRecord {
  ...
  async clear(): Promise<AnyRecord> {
    this.events = []; this.lastEventMs = 0; this.eventSeq = 0;
    try {
  ...
  async record(input: AnyRecord = {}): Promise<void> {
    try {
      ...
      if (signature !== this.lastSignature) created = buildChargingAuditEvents(this.snapshot, snapshot, { heartbeat: false });
      else if (heartbeat) created = buildChargingAuditEvents(snapshot, snapshot, { heartbeat: true });
      for (const event of created) {
        this.eventSeq += 1; event.id = `${now}-${this.eventSeq}`; this.events.push(event); this.lastEventMs = now;
      }

...
/**
 * Herstellerunabhaengige 0-W-Entscheidung fuer Speicher-Sollwerte.
 *
 * NexoWatt-Konvention:
 * - positive Leistung = Entladen
 * - negative Leistung = Laden
 * - 0 W = echter Stop-Befehl
 *
 * Ein transienter Zwischenwert, ein NVP im Zielband oder eine kurze Mess-/Budgetluecke
 * darf deshalb nicht automatisch zu einem Hardware-Stopp werden.
 */

declare const module: { exports: unknown };

export type StorageZeroWriteAction =
  | 'write-target'
  | 'write-stop'
  | 'hold-write'
  | 'hold-no-write'
...
export type StorageZeroWriteInput = {
  targetW?: unknown;
  lastTargetW?: unknown;
  source?: unknown;
  reason?: unknown;
  ...
  measurementGraceMs?: unknown;
  budgetZero?: boolean;
  budgetZeroConfirmed?: boolean;
  budgetZeroAgeMs?: unknown;
  budgetGraceMs?: unknown;
  nvpW?: unknown;
  nvpTargetW?: unknown;
  nvpDeadbandW?: unknown;
  feedForwardTargetW?: unknown;
  holdByNoWrite?: boolean;
};

export type StorageZeroWriteDecision = {
  action: StorageZeroWriteAction;
  outputW: number;
  holdW: number;
  explicitStop: boolean;
  status: string;
...
function reasonImpliesExplicitStop(reasonRaw: unknown, sourceRaw: unknown): boolean {
  const reason = text(reasonRaw).toLowerCase();
  const source = text(sourceRaw).toLowerCase();
  if (source === 'aus' || source === 'reserve') return true;
...
function holdDecision(input: StorageZeroWriteInput, lastTargetW: number, reason: string, status: string): StorageZeroWriteDecision {
 ...
 */
export function decideStorageZeroWrite(input: StorageZeroWriteInput = {}): StorageZeroWriteDecision {
  ...
  if (nvpW !== null) {
    if (lastTargetW < 0) return holdDecision(input, lastTargetW, reason

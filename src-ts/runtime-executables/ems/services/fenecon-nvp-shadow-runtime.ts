...
/**
 * RC42: Strikt read-only Runtime-Orchestrierung fuer den FENECON-NVP-Shadow.
 *
 * Diese Datei ist bewusst ausserhalb des historischen @ts-nocheck-
 * Speichermoduls typisiert. Sie darf nur Messwerte lesen und interne
 * Diagnose-States schreiben. Hardware-Writer, Reglerhoheit und SafetyEnvelope
 * bleiben im produktiven Speichercontroller.
 */
...
type StateFactory = (id: string, name: string, type: string, role: string, def?: unknown) => Promise<unknown>;
...
const FENECON_NVP_SHADOW_STATE_DEFINITIONS: ReadonlyArray<readonly [string, string, string, string, unknown?]> = [
...
async function ensureFeneconNvpShadowStates(mk: StateFactory): Promise<void> {
...
async function updateFeneconNvpShadowRuntime(host: AnyRecord, ctx: AnyRecord = {}): Promise<AnyRecord> {
  const safeSet = async (id: string, value: unknown): Promise<void> => {
      try {
  ...
  const clearNumericStates = async (): Promise<void> => {
      for (const id of [
  ...
  const setInactive = async (reason: unknown, extra: AnyRecord = {}): Promise<AnyRecord> => {
  ...
  try {
      ...
      if (!hybridEligible) return setInactive('not-fenecon-dc-hybrid');
      ...
      const intervalMs = Math.max(
          1000,
          Math.round(num(cfg.feneconNvpShadowIntervalSec, 5) * 1000),
      );
      ...
      const readFresh = (key: string): AnyRecord => {
          try {
              const entry = host.dp && typeof host.dp.getEntry === 'function'
                  ? host.dp.getEntry(key)
                  : null;
              if (!entry) return { key, mapped: false, value: null, ageMs: null, fresh: false };
      ...
      if (!centralNvp.fresh) centralNvp = readFresh('grid.powerW');
      if (!centralNvp.fresh) {
      ...
      if (totalPv.fresh) {
          ...
          selectedPvW = Math.max(0,
              (dcPv.fresh ? Math.abs(dcPv.value) : 0)
              + (acPv.fresh ? Math.abs(acPv.value) : 0));
          selectedPvAgeMs = Math.max(
              dcPv.fresh && Number.isFinite(Number(dcPv.ageMs)) ? Number(dcPv.ageMs) : 0,
              acPv.fresh && Number.isFinite(Number(acPv.ageMs)) ? Number(acPv.ageMs) : 0,
          );
      ...
      const zeroExportTargetW = clamp(
          strictFiniteNumber(
              cfg.feneconNvpShadowZeroExportTargetW
              ?? cfg.feneconZeroExportTargetW,
              80,
          ) ?? 80,
          0,
          1000,
      ...
      host._feneconNvpShadowLastSignature = shadowSignature;
      host._feneconNvpShadowLastDiag = diag;
      const n = (value: unknown): number | null => strictFiniteNumber(value, null) === null
          ? null
      ...
      const caught = error as { message?: unknown; stack?: unknown } | null;

...
/**
 * Pure 15-minute multi-EV target planner.
 *
 * The planner never writes hardware. It creates advisory power intents that are
 * applied later inside the existing charging-management single-writer path.
 * Missing PV, price or operating-strategy modules are valid inputs and degrade
 * to a conservative latest-start schedule.
 */

export type RequirementClass = 'must' | 'should' | 'can';
export type TariffMode = 'none' | 'manual' | 'automatic';
export type PlanAction = 'inactive' | 'complete' | 'wait' | 'charge' | 'blocked';
export type PlanSource = 'none' | 'pv' | 'price' | 'mixed' | 'deadline';

export interface ForecastPowerSegment {
  startMs?: number;
  endMs?: number;
  t?: number;
  dtMs?: number;
...
export interface ForecastPriceSegment {
  startMs: number;
  endMs: number;
  priceEurKwh: number;
}
...
export interface TariffPlannerConfig {
  active: boolean;
  fresh: boolean;
  mode: TariffMode;
  priority: 1 | 2 | 3;
  manualCheapThresholdEurKwh?: number | null;
  automaticCheapThresholdEurKwh?: number | null;
}
...
export interface TargetPlannerInput {
  ...
  slotMinutes?: number;
  reserveMs?: number;
  energySafetyFactor?: number;
  siteCapW: number;
  baseLoadW?: number;
  currentPvSurplusW?: number;
  pvPlanningSafetyPct?: number;
  pvCurve?: ForecastPowerSegment[];
  priceCurve?: ForecastPriceSegment[];
  tariff?: TariffPlannerConfig;
  /** False only for an explicit customer PV-only policy. Economic tariff waits are not hard policies. */
  gridPlanningAllowed?: boolean;
  goals: GoalRequest[];
}

export interface TargetPlanWindow {
  startMs: number;
  endMs: number;
  source: PlanSource;
...
export interface TargetPlan {
  ...
  plannedWh: number;
  plannedPvWh: number;
  plannedGridWh: number;
  unscheduledWh: number;
  latestStartMs: number;
  nextWindow: TargetPlanWindow | null;
  targetReachable: boolean;
  targetAtRisk: boolean;
  deadlineOverride: boolean;
  pvForecastUsed: boolean;
  priceForecastUsed: boolean;
  tariffMode: TariffMode;
  tariffPriority: number;
  fallbackMode: string;
}
...
interface PlannerSlot {
  ...
  siteCapW: number;
  siteUsedW: number;
  stationUsedW: Map<string, number>;
  pvAvailableW: number;
  pvUsedW: number;
...
interface MutableGoal {
  ...
  plannedPvWh: number;
  plannedGridWh: number;
  allocations: Map<number, SlotAllocation>;
}
...
function requirementRank(value: RequirementClass | undefined): number {
  return value === 'must' ? 3 : value === 'should' ? 2 : 1;
}

function segmentBounds(segment: ForecastPowerSegment): { startMs: number; endMs: number; powerW: number } | null {
...
function averagePower(curve: ForecastPowerSegment[] | undefined, startMs: number, endMs: number): number {
  if (!Array.isArray(curve) || endMs <= startMs) return 0;
  let weighted = 0;
  for (const segment of curve) {
    ...
    if (overlap > 0) weighted += normalized.powerW * overlap;
  }
  return weighted / Math.max(1, endMs - startMs);
}

function averagePrice(curve: ForecastPriceSegment[] | undefined, startMs: number, endMs: number): number | null {
  if (!Array.isArray(curve) || endMs <= startMs) return null;
  let weighted = 0;
  let covered = 0;
  for (const segment of curve) {
    ...
    if (!Number.isFinite(segmentStart) || !Number.isFinite(segmentEnd) || segmentEnd <= segmentStart || !Number.isFinite(price)) continue;
...
function percentile(values: number[], quantile: number): number | null {
  ...
  if (typeof lowValue !== 'number' || typeof highValue !== 'number') return null;
  return low === high ? lowValue : lowValue + (highValue - lowValue) * (position - low);
}

function createSlots(input: TargetPlannerInput, horizonEndMs: number): PlannerSlot[] {
  const nowMs = finite(input.nowMs, Date.now());
  const slotMs = Math.round(clamp(input.slotMinutes, 5, 60, 15) * 60_000);
...
function stationRemaining(slot: PlannerSlot, goal: GoalRequest): number {
...
function availablePower(slot: PlannerSlot, goal: GoalRequest, source: 'pv' | 'grid'): number {
  ...
  if (source === 'pv') available = Math.min(available, Math.max(0, slot.pvAvailableW - slot.pvUsedW));
  return Math.max(0, available);
}

function allocate(slot: PlannerSlot, goal: MutableGoal, source: 'pv' | 'grid'): number {
  ...
  if (stationKey) slot.stationUsedW.set(stationKey, (slot.stationUsedW.get(stationKey) || 0) + requestedW);
  if (source === 'pv') slot.pvUsedW += requestedW;

  const slotAllocation = slot.allocations.get(goal.request.id) || { totalW: 0, pvW: 0, gridW: 0 };
  slotAllocation.totalW += requestedW;
  if (source === 'pv') slotAllocation.pvW += requestedW;
  else slotAllocation.gridW += requestedW;
  ...
  if (source === 'pv') goalAllocation.pvW += requestedW;
  else goalAllocation.gridW += requestedW;
  ...
  if (source === 'pv') goal.plannedPvWh += allocatedWh;
  else goal.plannedGridWh += allocatedWh;
  return allocatedWh;
}

function currentAllocation(goal: MutableGoal, slots: PlannerSlot[], nowMs: number): SlotAllocation {
...
function nextWindow(goal: MutableGoal, slots: PlannerSlot[], nowMs: number, priceUsed: boolean): TargetPlanWindow | null {
  ...
  if (!first?.allocation) return null;
  ...
  for (let index = 1; index < selected.length; index += 1) {
    ...
    if (!current?.allocation || current.slot.index !== previousIndex + 1) break;
...
export function buildForecastAwareTargetPlans(input: TargetPlannerInput): TargetPlan[] {
  const nowMs = finite(input.nowMs, Date.now());
  const safetyFactor = clamp(input.energySafetyFactor, 1, 1.5, 1.05);
  const reserveMs = Math.max(0, finite(input.reserveMs, 10 * 60_000));
  const goals = (Array.isArray(input.goals) ? input.goals : [])
    ...
    .map((goal) => ({
      ...
      requirement: (goal.requirement === 'must' || goal.requirement === 'can' ? goal.requirement : 'should') as RequirementClass,
    }))
  ...
  const tariff: TariffPlannerConfig = {
  ...
  const priceValues = slots.map((slot) => slot.priceEurKwh).filter((value): value is number => Number.isFinite(value));
  ...
  const mutableGoals: MutableGoal[] = goals
  ...
  return mutableGoals.map((goal) => {
    ...
    const plannedNowW = current.totalW > 0
      ...
      : deadlineOverride && gridPlanningAllowed
        ...
        : 0;
    const action: PlanAction = goal.requiredWh <= 1
      ? 'complete'
      : plannedNowW > 0
        ? 'charge'
        : !targetReachable && !window
          ? 'blocked'
          : 'wait';
    const source: PlanSource = current.pvW > 0 && current.gridW > 0
      ? 'mixed'
      : current.pvW > 0
        ? 'pv'
        : current.gridW > 0

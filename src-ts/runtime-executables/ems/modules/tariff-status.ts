...
type PowerDirection = 'charge' | 'discharge' | 'idle' | 'unknown';
type StatusConfig = {
  intentMaxAgeMs: number;
  readbackDeadbandW: number;
  readbackGraceMs: number;
  readbackMaxAgeMs: number;
};
...
const directionOf = (valueW: unknown, deadbandW = 0): PowerDirection => {
  const n = finiteOrNull(valueW);
  if (n === null) return 'unknown';
  const deadband = Math.max(0, Number(deadbandW) || 0);
  if (n > deadband) return 'discharge';
  if (n < -deadband) return 'charge';
  return 'idle';
};

const formatPower = (valueW: unknown): string => {
  const n = roundedOrNull(valueW);
  if (n === null) return '—';
  return `${Math.abs(n)} W`;
};

const safeParseJson = (value: unknown, fallback: AnyRecord = {}): AnyRecord => {
...
const buildTariffBaseText = (tariff: AnyRecord = {}): string => {
...
const buildIntentReason = (tariff: AnyRecord = {}, intentW: unknown = 0): string => {
  const intentDirection = directionOf(intentW, 0);
  const tariffActive = boolValue(tariff.tarifAktiv, false);
  const netFeeActive = boolValue(tariff.netFeeEnabled, false) && String(tariff.netFeeMode || 'off').toLowerCase() !== 'off';
  const state = String(tariff.state || '').toLowerCase();

  if (!tariffActive && !netFeeActive) return 'Tarif und zeitvariables Netzentgelt sind aus';
  if (boolValue(tariff.dynamicTariffStale, false) && tariffActive && !netFeeActive) return 'Tarifdaten sind zu alt – keine neue Tarifaktion';
  if (!boolValue(tariff.storageWriterAvailable, true)) return cleanText(tariff.storageAuthorityReason || 'Kein beschreibbarer Speicher-Ausgang');
  if (boolValue(tariff.gridImportPreferred, false)) {
    if (boolValue(tariff.storageFullHold, false)) return 'Negativpreis aktiv, SoC-Ladeziel erreicht';
    if (intentDirection === 'charge') return 'Negativpreis aktiv – Speicher-Netzladen gewünscht';
  ...
  if (boolValue(tariff.storageFullHold, false)) return 'SoC-Ladeziel erreicht';
  if (String(tariff.netFeeMode || '').toUpperCase() === 'HT') return 'HT – Eigenverbrauch und Entladung bleiben freigegeben';
  if (String(tariff.netFeeMode || '').toUpperCase() === 'NT' && intentDirection === 'charge') return 'NT – Speicherladen gewünscht';
  if (state === 'guenstig' && intentDirection === 'charge') return 'Günstiger Tarif – Speicherladen gewünscht';
  if (state === 'teuer' && intentDirection === 'discharge') return 'Teurer Tarif – Speicherentladung gewünscht';
  if (intentDirection === 'charge') return 'Tarif-/Netzentgelt-Policy fordert Laden';
  if (intentDirection === 'discharge') return 'Tarif-/Netzentgelt-Policy fordert Entladen';
  return 'Tarif-/Netzentgelt-Policy fordert Warten';
};

const buildIntentPart = (intentW: unknown, intentReason: unknown): string => {
  const direction = directionOf(intentW, 0);
  if (direction === 'charge') return `Tarifwunsch Laden ${formatPower(intentW)}`;
  if (direction === 'discharge') return `Tarifwunsch Entladen ${formatPower(intentW)}`;
 ...
 */
function classifyTariffStorageStatus(input: AnyRecord = {}) {
  ...
  const blocked = !noWriter && (
    containsAny(writeStatusLower, [
      'blockiert',
      'blocked',
      'authority',
  ...
  if (!feedbackFresh) {
    ...
    if (actualDirection === 'charge') readbackStatus = 'confirmed-charging';
    else if (targetAgeMs < readbackGraceMs) readbackStatus = 'pending-charging';
    else if (actualDirection === 'idle') readbackStatus = 'no-charge-response';
    else readbackStatus = 'opposite-direction';
  } else if (finalDirection === 'discharge') {
    if (actualDirection === 'discharge') readbackStatus = 'confirmed-discharging';
    else if (targetAgeMs < readbackGraceMs) readbackStatus = 'pending-discharging';
    else if (actualDirection === 'idle') readbackStatus = 'no-discharge-response';
    else readbackStatus = 'opposite-direction';
  ...
  if (noWriter) gateStatus = 'no-writer';
  ...
  let status = 'inactive';
  if (active) {
    if (noWriter) status = 'no-writer';
    else if (blocked) status = 'blocked';
    else if (hold) status = 'hold';
    else if (writeFailed) status = 'write-failed';
    else if (finalDirection === 'charge') {
      if (readbackStatus === 'confirmed-charging') status = 'charging';
      else if (['no-charge-response', 'opposite-direction'].includes(readbackStatus)) status = 'mismatch';
      else status = 'charge-requested';
    } else if (finalDirection === 'discharge') {
      if (readbackStatus === 'confirmed-discharging') status = 'discharging';
      else if (['no-discharge-response', 'opposite-direction'].includes(readbackStatus)) status = 'mismatch';
      else status = 'discharge-requested';
    } else if (readbackStatus === 'confirmed-stop') status = 'waiting';
    else if (readbackStatus === 'stop-not-reached') status = 'stop-not-reached';
    else status = 'stop-requested';
  }
...
const buildEffectiveStorageText = (result: ReturnType<typeof classifyTariffStorageStatus>, context: AnyRecord = {}): string => {
  ...
  const detailParts = [];
  if (source) detailParts.push(`Quelle ${source}`);
...
class TariffStatusModule extends BaseModule {
  ...
  public _lastTargetSignature: string;
  public _targetSinceMs: number;
  ...
  async _readValues(ids: readonly string[]): Promise<Record<string, unknown>> {

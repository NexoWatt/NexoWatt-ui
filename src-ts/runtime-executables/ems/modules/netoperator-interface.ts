...
} from '../services/netoperator-canonical-model';
import {
  NetOperatorDriverRegistry,
  type NetOperatorDriverProfile,
  type NetOperatorQualityDescriptor,
  type NetOperatorReadDescriptor,
  type NetOperatorSignalMapping,
} from '../services/netoperator-driver-registry';
import { classifyQuality, NetOperatorModbusTcpConnector } from '../services/netoperator-modbus-tcp';

interface ModuleConfig {
  enabled?: boolean;
  mode?: 'off' | 'diagnostic' | 'commissioning' | 'active';
  commissioned?: boolean;
  installerApproved?: boolean;
  profileSource?: 'builtin' | 'custom';
  driverId?: string;
  customProfileJson?: string;
  signalMaxAgeSec?: number;
  lastValidHoldSec?: number;
  auditLimit?: number;
  failSafePolicy?: string;
  writebackEnabled?: boolean;
  transport?: {
    type?: 'modbus-tcp' | 'state-map' | 'modbus-rtu' | 'opc-ua' | 'iec-104' | 'iec-61850';
    host?: string;
    port?: number;
    unitId?: number;
    timeoutMs?: number;
...
function descriptorObjectId(descriptor: NetOperatorReadDescriptor | null | undefined): string {
  return safeText(descriptor && descriptor.objectId);
}

function transformStateValue(value: unknown, descriptor: NetOperatorReadDescriptor): unknown {
...
export class NetOperatorInterfaceModule {
  private adapter: any;
  private dp: any;
  private registry: NetOperatorDriverRegistry;
  private connector: NetOperatorModbusTcpConnector | null = null;
  private connectorSignature = '';
  private lastPollAt = 0;
  private lastSnapshot: CanonicalSnapshot | null = null;
  private lastRaw: AnyRecord = {};
  private audit: AnyRecord[] = [];
  private lastCommand: AnyRecord | null = null;
  private lastReceivedAt = 0;
  private lastValidAt = 0;

  constructor(adapter: any, dpRegistry: any) {
    this.adapter = adapter;
    this.dp = dpRegistry || null;
    this.registry = new NetOperatorDriverRegistry();
  }
   ...
   */
  private activation(cfg: ModuleConfig = this.config()): AnyRecord {
    ...
    const exportGuardRunMode = safeText(
  ...
  private strictNonNegative(value: unknown): number | null {
    if (value === null || value === undefined || (typeof value === 'string' && !value.trim())) return null;
    const parsed = Number(value);
    return Number.isFinite(parsed) && parsed >= 0 ? parsed : null;
  }

  private allowedExportPowerW(snapshot: CanonicalSnapshot): number | null {
    const command = snapshot && snapshot.command ? snapshot.command : null;
    if (!snapshot || snapshot.valid !== true || !command) return null;
    if (command.action === 'trip' || command.action === 'inhibit') return 0;
    if (command.binding !== true || command.action !== 'active-power-constraint') return null;
    const pLimitKw = this.strictNonNegative(canonicalValue(snapshot, 'grid.p.limit_kw'));
    if (pLimitKw !== null) return Math.round(pLimitKw * 1000);
    // Kanonischer Vertrag: positive P-Zielwerte bedeuten Einspeisung am NAP.
    const pTargetKw = this.strictNonNegative(canonicalValue(snapshot, 'grid.p.target_kw'));
    if (pTargetKw !== null) return Math.round(pTargetKw * 1000);
    const pTargetPct = this.strictNonNegative(canonicalValue(snapshot, 'grid.p.target_pct'));
    if (pTargetPct !== null && pTargetPct <= 100) {
      ...
      for (const value of ratedCandidates) {
        ...
        if (ratedPowerW !== null && ratedPowerW > 0) return Math.round(ratedPowerW * pTargetPct / 100);
      }
    }
    return null;
  }

  private fallbackExportPowerW(): number {
    ...
    for (const value of localCandidates) {
      ...
      if (parsed !== null) { localMaxW = Math.round(parsed); break; }
    }
    const fallbackCandidates = [cfg.fallbackExportPowerW, cfg.externalFallbackExportPowerW];
    for (const value of fallbackCandidates) {
      const parsed = this.strictNonNegative(value);
      if (parsed !== null) return Math.min(localMaxW, Math.round(parsed));
    }
    return localMaxW;
  }

  private envelopeQuality(snapshot: CanonicalSnapshot): string {
    if (!snapshot || snapshot.commOk !== true) return 'communication-error';
    if (snapshot.fresh !== true) return 'stale';
    if (snapshot.valid !== true) return 'invalid';
    if (snapshot.command?.binding !== true) return 'good-released';
    if (snapshot.command?.action === 'reactive-power-constraint') return 'good-no-active-power-limit';
    return 'good';
  }

  private async ensureObject(id: string, name: string, type: 'boolean' | 'number' | 'string', role = 'state', unit = ''): Promise<void> {
  ...
  async init(): Promise<void> {
    ...
    for (const definition of Object.values(CANONICAL_FIELDS)) {
      ...
      await this.ensureObject(`netoperator.${definition.key}`, definition.label, type as 'boolean' | 'number' | 'string', role, definition.unit || '');
    }
    await this.restoreAudit();
    this.adapter._netOperatorInterface = this;
  }

  private closeConnector(): void {
    if (this.connector) {
      try { this.connector.close(); } catch (_error) { /* ignore */ }
    }
    this.connector = null;
  ...
  stop(): void {
    ...
    if (this.adapter) {
      ...
      if (this.adapter._netOperatorInterface === this) this.adapter._netOperatorInterface = null;
    }
  }
  ...
  private appendAudit(snapshot: CanonicalSnapshot): void {
    ...
    const current = {
      ...
      allowedExportPowerW,
      values: {
        enable: canonicalValue(snapshot, 'grid.command.enable'),
        trip: canonicalValue(snapshot, 'grid.command.trip'),
        release: canonicalValue(snapshot, 'grid.command.release'),
        pLimitKw: canonicalValue(snapshot, 'grid.p.limit_kw'),
        pTargetKw: canonicalValue(snapshot, 'grid.p.target_kw'),
        pTargetPct: canonicalValue(snapshot, 'grid.p.target_pct'),
        qTargetKvar: canonicalValue(snapshot, 'grid.q.target_kvar'),
        cosPhi: canonicalValue(snapshot, 'grid.cosphi.target'),
      },
    };
    if (this.lastCommand && deepEqual(this.lastCommand, current)) return;
  ...
  private async readStateDescriptor(descriptor: NetOperatorReadDescriptor): Promise<{ state: AnyRecord; value: unknown; objectId: string }> {
    const objectId = descriptorObjectId(descriptor);
    if (!objectId) throw new Error('state-object-id-missing');
  ...
  private async readStateMap(profile: NetOperatorDriverProfile): Promise<PollResult> {
    ...
    for (const [key, mapping] of Object.entries(profile.signals || {})) {
      if (mapping.access === 'write') continue;
      const objectId = descriptorObjectId(mapping);
      if (!objectId) continue;
      const required = mapping.required === true || REQUIRED_READ_KEYS.includes(key);
      try {
        const primary = await this.readStateDescriptor(mapping as NetOperatorSignalMapping);
        successfulReads += 1;
        let quality = Number(primary.state.q) > 0 ? 'bad' : 'good';
        let timestamp = Number(primary.state.ts) || Date.now();
        const detail: AnyRecord = { objectId, value: primary.value, ack: primary.state.ack === true, ts: Number(primary.state.ts) || 0, q: primary.state.q ?? null };

        const qualityDescriptor = mapping.quality as NetOperatorQualityDescriptor | undefined;
        if (qualityDescriptor && descriptorObjectId(qualityDescriptor)) {
          ...
          quality = Number(qualityRead.state.q) > 0 ? 'bad' : classifyQuality(qualityRead.value, qualityDescriptor);
        ...
        if (mapping.timestamp && descriptorObjectId(mapping.timestamp)) {
          ...
          const parsed = strictTimestamp(timestampRead.value);
  ...
  private connectorFor(cfg: ModuleConfig, profile: NetOperatorDriverProfile): NetOperatorModbusTcpConnector {
    ...
    if (!this.connector || signature !== this.connectorSignature) {
      this.closeConnector();
      this.connector = new NetOperatorModbusTcpConnector({ host: safeText(transport.host), port: transport.port, unitId: transport.unitId, timeoutMs: transport.timeoutMs }, profile);
      this.connectorSignature = signature;
    }
    return this.connector;
  }

  private async publish(snapshot: CanonicalSnapshot, extra: {
    status: string;
    profile: NetOperatorDriverProfile | null;
    ...
    if (extra.transportOk) this.lastReceivedAt = snapshot.receivedAt;
    if (snapshot.valid) this.lastValidAt = snapshot.receivedAt;
    const cfg = this.config();
    const activation = this.activation(cfg);
    const maxAgeMs = this.maxAgeMs(cfg, extra.profile);
    const controllerTimestamp = strictTimestamp(canonicalValue(snapshot, 'controller.timestamp')) || snapshot.receivedAt;
    ...
    const source = safeText(canonicalValue(snapshot, 'controller.source') || snapshot.source || 'EZA-/Parkregler');
    ...
    await this.adapter.setStateAsync('netoperator.diagnostics.canonicalQualityJson', JSON.stringify(Object.fromEntries(
      Object.entries(snapshot.values || {}).map(([key, entry]) => [key, {
    ...
    for (const [key, definition] of Object.entries(CANONICAL_FIELDS)) {
      ...
      if (!entry || entry.valid !== true) continue;
      const value = entry.value;
      if (definition.type === 'boolean' && typeof value === 'boolean') await this.adapter.setStateAsync(`netoperator.${key}`, value, true);
      else if ((definition.type === 'number' || definition.type === 'datetime') && typeof value === 'number') await this.adapter.setStateAsync(`netoperator.${key}`, value, true);
      else if (value !== null && value !== undefined) await this.adapter.setStateAsync(`netoperator.${key}`, String(value), true);
    }
    this.appendAudit(snapshot);
  }

  private maxAgeMs(cfg: ModuleConfig, profile: NetOperatorDriverProfile | null): number {
  ...
  async tick(): Promise<void> {
    ...
    this.registry.load();
    const resolved = this.registry.resolve(cfg as AnyRecord);
    if (!resolved.profile || !resolved.validation.ok) {
      this.closeConnector();
      const snapshot = buildCanonicalSnapshot({ commOk: false, source: 'profile', driverId: safeText(cfg.driverId), maxAgeMs: this.maxAgeMs(cfg, resolved.profile), errors: [resolved.error || 'driver-profile-invalid'] });
    ...
    if (this.lastSnapshot && Date.now() - this.lastPollAt < pollIntervalMs) return;
    this.lastPollAt = Date.now();
    const transportType = safeText(cfg.transport?.type || resolved.profile.defaultProtocol || 'modbus-tcp').toLowerCase();
    let pollResult: PollResult;
    if (!resolved.profile.protocols.includes(transportType as any)) {
      this.closeConnector();
  ...
  async testConnection(configOverride?: ModuleConfig): Promise<AnyRecord> {
    const cfg = configOverride && typeof configOverride === 'object' ? configOverride : this.config();
    this.registry.load();
    const resolved = this.registry.resolve(cfg as AnyRecord);
    if (!resolved.profile || !resolved.validation.ok) return { ok: false, error: resolved.error || 'driver-profile-invalid', validation: resolved.validation };
    const transportType = safeText(cfg.transport?.type || resolved.profile.defaultProtocol || 'modbus-tcp').toLowerCase();
    ...
    if (!resolved.profile.protocols.includes(transportType as any)) {
      ...
      const transport = cfg.transport || {};
      const connector = new NetOperatorModbusTcpConnector({ host: safeText(transport.host), port: transport.port, unitId: transport.unitId, timeoutMs: transport.timeoutMs }, resolved.profile);
      try { result = await connector.poll(); } finally { connector.close(); }
    } else {

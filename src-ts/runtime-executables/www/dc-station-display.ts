 ...
 * RC66 responsive multi-LP station display; generated through the canonical
 * runtime sync and restricted to charging-management command intents.
 */
...
(function () {
  ...
  type DisplayPayload = AnyRecord;
  type RenderOptions = { error?: string };
  type DisplayMessage = AnyRecord & {
    level?: string;
    title?: string;
    message?: string;
    text?: string;
  ...
  type RequestError = Error & { status?: number; data?: AnyRecord | null };
  type TimerHandle = ReturnType<typeof setTimeout> | null;
  type I18nCatalog = Record<string, Record<string, string>>;


  const app = document.getElementById('stationDisplayApp') as HTMLElement | null;
  let token = '';
  let lastPayload: DisplayPayload | null = null;
  let busyKey = '';
  let toastTimer: TimerHandle = null;
  let refreshTimer: TimerHandle = null;
  let heartbeatTimer: TimerHandle = null;
  let lastOkTs = 0;
  let connectionLost = false;
  let manualLanguage = '';

  const I18N: I18nCatalog = {
    de: {
  ...
  function fmtKw(w: unknown): string {
    const n = Number(w);
  ...
  function fmtKwh(v: unknown): string { const n = Number(v); return Number.isFinite(n) ? n.toFixed(2).replace('.', ',') + ' kWh' : '0,00 kWh'; }
  function fmtEur(v: unknown): string { const n = Number(v); return Number.isFinite(n) ? n.toFixed(2).replace('.', ',') + ' €' : '0,00 €'; }
  function fmtPrice(v: unknown): string { const n = Number(v); return Number.isFinite(n) ? n.toFixed(3).replace('.', ',') + ' €/kWh' : '—'; }
  ...
  function fmtDateTime(ts: unknown): string {
    const n = Number(ts); if (!Number.isFinite(n) || n <= 0) return '—';
    try { return new Date(n).toLocaleString(lang() === 'en' ? 'en-GB' : (lang() === 'nl' ? 'nl-NL' : 'de-DE'), { dateStyle: 'short', timeStyle: 'medium' }); } catch (_e) { return fmtTime(n); }
  }
  ...
  function clamp(n: unknown, min: number, max: number): number { const v = Number(n); return Number.isFinite(v) ? Math.max(min, Math.min(max, v)) : min; }
  function safeArray<T = AnyRecord>(raw: unknown): T[] { return Array.isArray(raw) ? raw as T[] : []; }
  function getToken(): string { const m = location.pathname.match(/\/display\/station\/([^/?#]+)/i); const value = m?.[1]; return value ? decodeURIComponent(value) : ''; }
  ...
  function statusLabel(status: unknown): string {
    ...
    if (s === 'unavailable') return t('inactive');
    return t('offline');
  }
  ...
  function tariffStateLabel(state: unknown): string {
    const raw = String(state || '').trim().toLowerCase();
    if (['cheap', 'guenstig', 'günstig'].includes(raw)) return t('tariffCheap');
    if (['neutral', 'normal'].includes(raw)) return t('tariffNeutral');
    if (['expensive', 'teuer'].includes(raw)) return t('tariffExpensive');
    if (!raw || raw === 'off') return t('tariffInactive');
    return t('tariffUnknown');
  }

  function addUnique(list: DisplayMessage[], entry: DisplayMessage, key?: string): void {
    if (!entry) return;
    const signature = String(key || `${entry.level || ''}|${entry.title || ''}|${entry.message || entry.text || ''}`);
    if (list.some((item) => item && item.__signature === signature)) return;
    list.push(Object.assign({ __signature: signature }, entry));
  }

  function derivePresentationModel(payload: DisplayPayload): { summary: AnyRecord; warnings: DisplayMessage[]; decisionLines: DisplayMessage[] } {
    ...
    const decisionLines: DisplayMessage[] = [];
    ...
    connectors.forEach((connector, index) => {
      ...
      if (ctl.userEnabled !== false && mode && !activeModes.includes(mode)) activeModes.push(mode);
      if (connector.charging || connector.status === 'charging' || Number(connector.powerW) > 100) chargingCount += 1;
      if (connector.plugged) pluggedCount += 1;
      if (ctl.goalEnabled) goalCount += 1;
      if (ctl.goalTariffOverride) goalOverrideCount += 1;
      ...
      if (connector.meterStale) addUnique(warnings, { level: 'warning', title: lpName, message: t('meterDataStale') }, `lp-meter-${connector.id}`);
    ...
    if (connectionLost || station.displayOnline === false) addUnique(warnings, { level: 'error', title: t('communication'), message: t('connectionLost') }, 'station-offline');
    if (station.maintenanceMode) addUnique(warnings, { level: 'warning', title: t('maintenance'), message: t('maintenanceActive') }, 'station-maintenance');
    if (station.displayWarning) addUnique(warnings, { level: 'warning', title: t('warningsTitle'), message: String(station.displayWarning) }, 'station-warning');
    if (control.staleMeter) addUnique(warnings, { level: 'error', title: t('communication'), message: t('meterDataStale') }, 'global-meter-stale');
    if (control.staleBudget) addUnique(warnings, { level: 'warning', title: t('stationLimit'), message: t('budgetDataStale') }, 'global-budget-stale');
    if (control.failsafeDetails) addUnique(warnings, { level: 'warning', title: 'Safety', message: String(control.failsafeDetails) }, 'global-failsafe');
    if (tariff.active && tariff.fresh === false) addUnique(warnings, { level: 'warning', title: t('tariff'), message: t('dataStale') }, 'tariff-stale');
    ...
    if (goalCount > 0) addUnique(decisionLines, { level: goalOverrideCount > 0 ? 'warning' : 'info', text: goalOverrideCount > 0 ? t('goalOverride') : `${t('goalConfigured')} (${goalCount})` }, 'decision-goal');
    if (control.para14aBinding || control.para14aActive) addUnique(decisionLines, { level: 'warning', text: t('para14aLimitActive') }, 'decision-para14a');
    else if (control.gridCapBinding) addUnique(decisionLines, { level: 'warning', text: t('gridLimitActive') }, 'decision-grid');
    else if (control.phaseCapBinding) addUnique(decisionLines, { level: 'warning', text: t('phaseLimitActive') }, 'decision-phase');
    ...
    connectors.forEach((connector) => {
      ...
      if (raw && !uniqueReasons.includes(raw)) uniqueReasons.push(raw);
    });
    ...
    let shortDecision = '';
    if (errorCount > 0) { const firstError = warnings.find((entry) => entry.level === 'error'); shortDecision = `${t('stationFault')}: ${firstError?.message || t('error')}`; }
    else if (control.para14aBinding || control.para14aActive) shortDecision = t('para14aLimitActive');
    else if (control.gridCapBinding) shortDecision = t('gridLimitActive');
    else if (control.phaseCapBinding) shortDecision = t('phaseLimitActive');
    else if (chargingCount > 0) shortDecision = `${t('stationCharging')}: ${modeText}, ${t('tariff')} ${tariffStateLabel(tariff.state)}.`;
    else if (pluggedCount > 0) shortDecision = `${t('stationWaiting')}: ${uniqueReasons[0] || (site.pvAvailable ? t('pvDecisionAvailable') : t('pvDecisionUnavailable'))}`;
    else shortDecision = `${t('stationReady')}: ${t('noVehicle')}`;
  ...
  function withPresentationModel(payload: DisplayPayload): DisplayPayload {
  ...
  function showStatus(title: unknown, message?: unknown, cls = ''): void {
    if (!app) return;
  ...
  function toast(message: unknown): void {
    let el = document.querySelector<HTMLElement>('.nw-display-toast');
    if (!el) { el = document.createElement('div'); el.className = 'nw-display-toast'; document.body.appendChild(el); }
    el.textContent = String(message == null ? '' : message); el.classList.add('show');
    if (toastTimer) clearTimeout(toastTimer);
    toastTimer = setTimeout(() => el.classList.remove('show'), 2200);
  }

  async function fetchJson(url: string, opts?: RequestInit): Promise<AnyRecord> {
    const res = await fetch(url, Object.assign({ cache: 'no-store' }, opts || {}));
    let data: AnyRecord | null = null; try { data = await res.json() as AnyRecord; } catch (_e) { data = null; }
    if (!res.ok || !data || data.ok === false) {
      const err = new Error((data && (data.message || data.error)) || ('HTTP ' + res.status)) as RequestError; err.status = res.status; err.data = data; throw err;
    }
    return data;
  }

  function refreshDelay(payload: DisplayPayload): number {
  ...
  function scheduleRefresh(delay = 2500): void { if (refreshTimer) clearTimeout(refreshTimer); refreshTimer = setTimeout(refresh, clamp(delay || 2500, 1000, 30000)); }
  function scheduleHeartbeat(delay = 10000): void {
  ...
  async function refresh(): Promise<void> {
    try {
      ...
      if (!token) { showStatus(t('unavailable'), 'Kein Display-Token in der URL.', 'nw-display-status--error'); return; }
      const data = await fetchJson('/api/display/station/' + encodeURIComponent(token));
      lastPayload = data; lastOkTs = Date.now(); connectionLost = false;
      ...
      render(data); scheduleRefresh(refreshDelay(data));
    } catch (error) {
      const e = error as RequestError;
      const data = e.data;
      if (data && data.error === 'eos_required') { showStatus(t('eosRequired'), data.message || t('eosRequired'), 'nw-display-status--error'); return; }
      connectionLost = true;
      if (lastPayload) render(lastPayload, { error: (e && e.message) || t('connectionLost') });
      else showStatus(t('unavailable'), (e && e.message) || t('loading'), 'nw-display-status--error');
  ...
  async function heartbeat(): Promise<void> {
    try {
      ...
      await fetchJson('/api/display/station/' + encodeURIComponent(token) + '/heartbeat', {
        method: 'POST', body: JSON.stringify({ ts: Date.now(), width: window.innerWidth || 0, height: window.innerHeight || 0, visibility: document.visibilityState || 'visible', language: lang(), appVersion: '0.8.195' }),
        headers: { 'Content-Type': 'application/json' },
      });
    } catch (_e) {}
  }

  async function sendCommand(lp: string | null, action: string | null, mode: string | null, extra: AnyRecord = {}): Promise<void> {
    ...
    busyKey = `${lp}:${action}:${mode || ''}`; render(lastPayload);
    try {
      ...
      if (data && data.payload) { lastPayload = data.payload; lastOkTs = Date.now(); connectionLost = false; render(data.payload); }
    } catch (error) { const e = error as RequestError; toast(e.message || 'Fehler'); }
    finally { busyKey = ''; setTimeout(refresh, 500); }
  }

  function languageSwitchHtml(station: AnyRecord): string {
  ...
  function bannerHtml(payload: DisplayPayload, opts: RenderOptions = {}): string {
    const station = payload.station || {}; const banners = [];
  ...
  function statusChip(icon: string, title: string, value: unknown, tone = 'info', detail?: unknown): string {
  ...
  function renderStatusStrip(payload: DisplayPayload): string {
    const site = payload.site || {}; const station = payload.station || {}; const summary = site.summary || {}; const tariff = site.tariff || {}; const control = site.control || {};
    const activeModes = safeArray(summary.activeModes); const mode = !control.active ? t('controlDisabled') : (activeModes.length > 1 ? t('mixed') : modeLabel(activeModes[0] || control.mode || 'auto'));
    const goalCount = Number(summary.goalCount || 0); const warningCount = Number(summary.warningCount || 0); const errorCount = Number(summary.errorCount || 0);
  ...
  function summaryCard(icon: string, title: string, value: unknown, detail?: unknown, tone = 'info', extra = ''): string {
  ...
  function renderSummary(payload: DisplayPayload): string {
    const site = payload.site || {}; const operator = payload.operator || {}; const station = payload.station || {}; const summary = site.summary || {}; const control = site.control || {};
    const totalToday = Number(operator.energyTodayKwh || 0); const solarToday = Number(operator.solarEnergyTodayKwh || 0);
    const solarShare = totalToday > 0 ? Math.round((solarToday / totalToday) * 100) : Number(operator.solarShareTodayPercent || 0);
    const activeModes = safeArray(summary.activeModes); const modeText = !control.active ? t('controlDisabled') : (activeModes.length > 1 ? t('mixed') : modeLabel(activeModes[0] || control.mode || 'auto'));
  ...
  function connectorLayout(connectors: AnyRecord[]): { count: number; columns: number; rows: number; density: string } {
  ...
  function isActiveValue(current: unknown, expected: unknown): boolean { return String(current || '').toLowerCase() === String(expected || '').toLowerCase(); }
  function controlButton(lp: unknown, action: unknown, mode: unknown, label: unknown, active: boolean, disabled: boolean, extraAttrs = ''): string {
  ...
  function renderLpControls(c: AnyRecord, station: AnyRecord, disabled: boolean): string {
    const lp = String(c && c.id || 'lp'); const ctl = c && c.controls && typeof c.controls === 'object' ? c.controls : {};
    const userMode = String(ctl.userMode || c.userMode || c.mode || 'auto').toLowerCase(); const userEnabled = ctl.userEnabled !== false;
    ...
    const storageOn = ctl.userStorageAssistEnabled === true; const goalOn = ctl.goalEnabled === true;
    const phaseSupported = !!(c && c.isAc && ctl.phaseSwitchSupported); const phaseMode = String(ctl.userPhaseMode || 'auto-pv').toLowerCase();
  ...
  function renderConnector(c: AnyRecord, station: AnyRecord): string {
    const status = String(c.status || 'unavailable'); const lp = String(c.id || 'lp'); const busy = !!busyKey && busyKey.startsWith(lp + ':');
    ...
    const modes = Array.isArray(c.allowedModes) ? c.allowedModes : ['solar', 'fast']; const solarAllowed = modes.includes('solar'); const fastAllowed = modes.includes('fast');
    ...
    const vehicleConnected = !!c.plugged; const diag = c.diagnostics || {}; const ctl = c.controls || {};
    const vehicleSoc = Number(c.vehicleSocPct); const vehicleDetail = Number.isFinite(vehicleSoc) ? `SoC ${Math.round(vehicleSoc)} %` : (c.effectiveStatus || c.rawStatus || '');
  ...
  function renderDecisionPanel(payload: DisplayPayload): string {
    const site = payload.site || {}; const lines = safeArray(site.decisionLines).slice(0, 5);
  ...
  function renderWarningsPanel(payload: DisplayPayload, opts: RenderOptions = {}): string {
    const site = payload.site || {}; const warnings = safeArray(site.warnings).slice(0, 4);
    if (connectionLost) warnings.unshift({ level: 'error', title: t('communication'), message: (opts && opts.error) || t('connectionLost') });
  ...
  function render(payload: DisplayPayload, opts: RenderOptions = {}): void {
    if (!app || !payload) return;
    const viewPayload = withPresentationModel(payload);
    const station = viewPayload.station || {}; const site = viewPayload.site || {}; const display = viewPayload.display || {}; const connectors = Array.isArray(viewPayload.connectors) ? viewPayload.connectors : [];
    const layout = connectorLayout(connectors); const countBucket = layout.count >= 5 ? 'many' : String(layout.count);
    ...
    app.dataset.density = layout.density; app.dataset.connectorCount = String(layout.count);
    app.style.setProperty('--lp-columns', String(layout.columns)); app.style.setProperty('--lp-rows', String(layout.rows));
    ...
    <footer class="nw-display-footer"><span>NexoWatt EOS · Energy Operation System</span><span>${escapeHtml(display.apiVersion || '0.8.195')}</span><span>${escapeHtml(t('directHardwareWrite'))}</span></footer>`;
    ...
    app.querySelectorAll<HTMLElement>('[data-lang]').forEach((btn) => btn.addEventListener('click', () => { manualLanguage = String(btn.getAttribute('data-lang') || '').toLowerCase(); document.documentElement.lang = manualLanguage || 'de'; render(lastPayload || payload, opts); }));
  }

  token = getToken();
  showStatus('NexoWatt Charge', t('loading'), 'nw-display-status--loading');
  refresh(); heartbeat(); scheduleHeartbeat(10000);
})();

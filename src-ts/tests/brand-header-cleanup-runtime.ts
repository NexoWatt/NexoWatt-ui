...
function runBrandHeaderCleanupTest(): void {
  ...
  assert(read('src-ts/runtime-executables/www/sw.ts').includes("nexowatt-cache-v304"), 'Service-Worker-TS-Quelle muss Cache v303 nutzen.');
  assert(read('www/sw.js').includes("nexowatt-cache-v304"), 'Generierter Service Worker muss Cache v303 nutzen.');
}

runBrandHeaderCleanupTest();
console.log('[ts-brand-header-cleanup] OK: 0.8.3-Topbar zeigt NexoWatt ohne EMS-Zusatz, Version/Manifest/Cache sind konsistent.');

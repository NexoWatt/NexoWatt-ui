 ...
 *
 * Zweck:
 * Prüft den 0.8.2-Branding-Schritt vollständig aus TypeScript: Die sichtbare
...
function runBrandHeaderCleanupTest(): void {
  ...
  assert(io.common?.version === '0.8.2', 'io-package.json common.version muss 0.8.2 sein.');
  assert(Boolean(io.common?.news?.['0.8.2']), 'io-package.json common.news braucht einen 0.8.2-Eintrag.');
  assert(Object.keys(io.common?.news ?? {}).length <= 7, 'io-package.json common.news darf maximal 7 Einträge haben.');
  assert(manifest.version === '0.8.2', 'manifest.webmanifest muss Version 0.8.2 haben.');
  ...
  assert(read('src-ts/runtime-executables/www/sw.ts').includes("nexowatt-cache-v303"), 'Service-Worker-TS-Quelle muss Cache v303 nutzen.');
  assert(read('www/sw.js').includes("nexowatt-cache-v303"), 'Generierter Service Worker muss Cache v303 nutzen.');
}

runBrandHeaderCleanupTest();
console.log('[ts-brand-header-cleanup] OK: 0.8.2-Topbar zeigt NexoWatt ohne EMS-Zusatz, Version/Manifest/Cache sind konsistent.');

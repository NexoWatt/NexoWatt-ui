// @ts-nocheck
/**
 * TypeScript-Parallelspiegel: ems/services/energy-origin-ledger-runtime.js
 *
 * Zweck:
 * Diese Datei ist die TypeScript-Vorbereitung der bestehenden JavaScript-Runtime-Datei.
 * Sie wird noch nicht produktiv ausgeführt. Die produktive Quelle bleibt vorerst:
 * ems/services/energy-origin-ledger-runtime.js
 *
 * Zusammenhang:
 ...
 *
 * Original-Hash: 0ed88df94bd3aa391df285a6c8f1e898b23aecb04668283d0fcb81b9b684cd2a
 */
 ...
 *
 * Quelle: src-ts/runtime-executables/ems/services/energy-origin-ledger-runtime.ts
 * Quell-Hash: sha256:d77ca1f30d001805ae80898d063a91dd52ae976bc6474f080811afb14a4d56e7
 * Erzeugung: npm run sync:ts-runtime-executables
 *
 * Zweck:
 * Diese JavaScript-Datei ist das ausführbare Build-Artefakt für ems/services/energy-origin-ledger-runtime.js.
 ...
 */
'use strict';
const { ORIGIN_LEDGER_VERSION, normalizeEdition, normalizeOriginConfig, emptyStorageInventory, normalizeInventory, meterSampleFromState, interpolateCumulativeSample, calculateOriginInterval, aggregateIntervals, intervalBounds, } = require('./energy-origin-accounting');
/**
 * Code-Teil: EnergyOriginLedgerRuntime
 *
 * Zweck:
...
class EnergyOriginLedgerRuntime {
    ...
    async _ensureStates() {
        ...
        if (!a || typeof a.setObjectNotExistsAsync !== 'function')
            return;
/**
 * Code-Teil: channel
 *
 ...
 */
    ...
    async _readJsonState(id, fallback) {
        try {
            ...
            if (!st || st.val === undefined || st.val === null || st.val === '')
                return fallback;
    ...
    async _primeFromStates() {
        ...
        if (!Array.isArray(this._recentIntervals))
            this._recentIntervals = [];
        ...
        this._storageInventory = normalizeInventory(await this._readJsonState('energyLedger.origin.storageInventoryJson', {}), this._config().storage);
        this._configHistory = await this._readJsonState('energyLedger.origin.configHistoryJson', []);
        if (!Array.isArray(this._configHistory))
            this._configHistory = [];
    }
    ...
    _evidenceReady(config, lastInterval) {
        if (!lastInterval || !lastInterval.evidence)
            return false;
        if (config.country === 'DE')
            return lastInterval.evidence.de && lastInterval.evidence.de.ready === true;
        if (config.country === 'NL')
            return lastInterval.evidence.nl && lastInterval.evidence.nl.ready === true;
    ...
    async _process(_trigger = 'tick') {
        ...
        try {
            ...
            if (configChanged) {
                ...
                if (!this._storageInventory || !this._storageInventory.schema)
                    this._storageInventory = emptyStorageInventory(config.storage);
            ...
            if (this.adapter && this.adapter.log && typeof this.adapter.log.warn === 'function')
                this.adapter.log.warn(`[energy-origin-ledger] ${message}`);
        }
    }
}

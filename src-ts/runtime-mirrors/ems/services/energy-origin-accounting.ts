// @ts-nocheck
/**
 * TypeScript-Parallelspiegel: ems/services/energy-origin-accounting.js
 *
 * Zweck:
 * Diese Datei ist die TypeScript-Vorbereitung der bestehenden JavaScript-Runtime-Datei.
 * Sie wird noch nicht produktiv ausgeführt. Die produktive Quelle bleibt vorerst:
 * ems/services/energy-origin-accounting.js
 *
 * Zusammenhang:
 ...
 *
 * Original-Hash: e0b132ae472e492b9ac849bfe04643dbc12b2c699f7dd4984aeb581b689cf282
 */
 ...
 * Quell-Hash: sha256:551b5ae021c2b02094ff8c5c7080d24f20fe41afdbc02474e0a5724cd61720da
...
/**
 * Code-Teil: normalizeChargePoint
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
...
/**
 * Code-Teil: normalizeOriginConfig
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
...
function normalizeOriginConfig(rawConfig, editionRaw = 'none') {
  ...
  for (const cp of chargePoints) {
    if (!cp.energyMeterKwhId) continue;
...
/**
 * Code-Teil: emptyStorageInventory
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
...
/**
 * Code-Teil: meterSampleFromState
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
...
/**
 * Code-Teil: interpolateCumulativeSample
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
...
function interpolateCumulativeSample(previous, current, boundaryTs) {
  ...
  if (!p && !c) return null;
  if (!p) return { ...c, ts: boundaryTs };
  if (!c) return { ...p, ts: boundaryTs };
  if (c.ts <= p.ts || boundaryTs <= p.ts) return { ...p, ts: boundaryTs };
  if (boundaryTs >= c.ts) return { ...c, ts: boundaryTs };
  const delta = c.valueKwh - p.valueKwh;
  if (!Number.isFinite(delta) || delta < -EPS_KWH) return { ...p, ts: boundaryTs, resetDetected: true };
...
function deltaFromSamples(start, end) {
  ...
  if (delta < -EPS_KWH) return { valid: false, deltaKwh: 0, reason: 'counter-reset' };
  return { valid: true, deltaKwh: Math.max(0, round(delta, 6)), reason: delta < 0 ? 'rounding' : 'ok' };
}

/**
 * Code-Teil: splitProportional
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
...
function addStorageCharge(inventoryRaw, chargeSources, chargeKwh, efficiencyPct) {
  ...
  if (assigned < storedTotal - EPS_KWH) addedUnknown += storedTotal - assigned;
  const next = {
    schema: ORIGIN_INVENTORY_VERSION,
...
function roleDeltaMap(meters, startSamples, endSamples) {
  ...
  for (const meter of meters || []) {
    ...
    if (!byRole[meter.role]) byRole[meter.role] = [];
...
/**
 * Code-Teil: sourcePoolForDirectLoads
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
...
/**
 * Code-Teil: allocateSourcesToLoads
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
 ...
 */
  const assignByWeights = (source, amount, selectedRows) => {
    ...
    while (left > EPS_KWH && eligible.some(row => remaining[row.id] > EPS_KWH)) {
      ...
      if (assignedRound <= EPS_KWH) break;
...
function buildQuality(config, meterResults, balance) {
  ...
  let status = 'complete';
  if (missingRoles.length || invalidMeters.length) status = 'incomplete';
  if (imbalancePct > 10) status = 'invalid';
  else if (imbalancePct > 2 && status === 'complete') status = 'partial';
  return {
    status,
    missingRoles,
    invalidMeters,
...
/**
 * Code-Teil: calculateOriginInterval
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
...
function calculateOriginInterval({ config: rawConfig, startSamples, endSamples, storageInventory, previousHash = '', startTs, endTs, edition }) {
  ...
  for (const row of evcsRows) {
    ...
    for (const [key, value] of Object.entries(sources)) evcsBreakdown[key] = round(finiteNumber(evcsBreakdown[key], 0) + finiteNumber(value, 0), 6);
  }

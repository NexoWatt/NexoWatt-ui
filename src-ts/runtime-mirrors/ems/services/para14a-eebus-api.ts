 ...
 *
 * Original-Hash: 6b4dfeb85f06fe4b5b67d5735b394081bfa586ea389ea339b31529bdd7c90aee
 */
...
class Para14aEebusDirectApi {
  ...
  _handleControl(obj) {
    ...
    const tickQueued = this.adapter?._nwRequestImmediateEmsTick?.(
      `eebus-cls:${packet.commandId}`,
      0,
    ) === true;
  ...
  _validatePacket(message, from) {
    ...
    if (!commandId) return { ok: false, reason: 'missing-command-id' };
  ...
  _armHelloWatchdog(sourceInstance, helloAtMs) {
    ...
    this.helloWatchdog = this._setTimer(() => {
      this.helloWatchdog = null;
      if (!this.initialized) return;
      if (this.sourceInstance !== sourceInstance || this.lastHelloAtMs !== helloAtMs) return;
  ...
  _supersedePending(nextCommandId) {
    for (const [commandId, pending] of this.pending.entries()) {
      ...
      const feedback = this._terminalFeedback(
        pending,
        'superseded',
        `Superseded by newer CLS command ${nextCommandId}.`,
        now,
      );
  ...
  _handleImplementationTimeout(pending) {
    ...
    const feedback = this._terminalFeedback(
      pending,
      'failed',
      `Central EOS implementation feedback was not completed before the ${pending.packet.implementationTimeoutMs || DEFAULT_IMPLEMENTATION_TIMEOUT_MS} ms bridge deadline.`,
      now,
    );
    feedback.details = {
      ...(feedback.details || {}),
  ...
  async _ensureObjects() {
    ...
    for (const [id, object] of Object.entries(objects)) {
      try { await this.adapter.setObjectNotExistsAsync(id, object); } catch (_error) {}
    }
  }

// @ts-nocheck
/**
 * TypeScript-Parallelspiegel: ems/modules/ai-advisor.js
 *
 * Zweck:
 ...
 *
 * Original-Hash: 0bcc7d03a490e34c0ccc6c37fd47e04004c53b3fd75fb5c21de95062d107ab26
 */
...
/**
 * Ai-Advisor Runtime-Migrationshinweis (DE):
 * Diese große Runtime-Spiegeldatei bleibt im normalen Projekt-Typecheck vorerst mit
 * @ts-nocheck geschützt. In 0.7.91 wird sie gezielt typisiert vorbereitet:
 * Adapter-Zugriff, Datenpunkt-Registry, KI-Konfiguration, Vorschläge, Tagesplan,
 * Lern-/Anomaliewerte und Snapshot-Formen bekommen erste TypeScript-Verträge.
 *
 * Wichtig:
 * Die produktive Runtime bleibt ems/modules/ai-advisor.js. Dieser Spiegel dient als
 * sicherer Migrationsraum und wird zusätzlich in einem gelockerten Check ohne
 * @ts-nocheck kompiliert. Die KI bleibt rein beratend und darf keine Aktoren schalten.
 */

declare const require: any;
declare const module: any;

type AiAdvisorUnknownRecord = Record<string, any>;

type AiAdvisorStateValue = {
    val?: unknown;
    value?: unknown;
    ts?: number;
    lc?: number;
...
type AiAdvisorStateCache = Record<string, AiAdvisorStateValue | undefined>;

type AiAdvisorLogLike = {
    debug?: (...args: any[]) => void;
    info?: (...args: any[]) => void;
    warn?: (...args: any[]) => void;
    error?: (...args: any[]) => void;
...
type AiAdvisorAdapterLike = {
    [key: string]: any;
    config?: AiAdvisorUnknownRecord;
    stateCache?: AiAdvisorStateCache;
    log?: AiAdvisorLogLike;
    namespace?: string;
    evcsCount?: number;
    evcsList?: any[];
    setObjectNotExistsAsync?: (id: string, obj: any) => Promise<unknown>;
    extendObjectAsync?: (id: string, obj: any) => Promise<unknown>;
    setStateAsync?: (id: string, state: any, ack?: boolean) => Promise<unknown>;
...
type AiAdvisorDpRegistryLike = {
    getNumberFresh?: (key: string, staleMs?: number, fallback?: number | null) => number | null;
    getNumber?: (key: string, fallback?: number | null) => number | null;
    getBoolean?: (key: string, fallback?: boolean) => boolean;
    getString?: (key: string, fallback?: string) => string;
};

type AiAdvisorPriority = 'info' | 'warning' | 'action' | 'critical';
type AiAdvisorSeverity = 'success' | 'info' | 'warning' | 'critical' | 'neutral';
type AiAdvisorMode = 'balanced' | 'cost' | 'autarky' | 'co2' | 'comfort' | 'peak';

type AiAdvisorCategories = {
    [key: string]: boolean;
    tariff: boolean;
    pv: boolean;
    storage: boolean;
...
type AiAdvisorRuntimeConfig = {
    enabled: boolean;
    customerEnabled: boolean;
    configuredEnabled: boolean;
    advisoryOnly: true;
    showInLive: boolean;
    optimizationMode: AiAdvisorMode;
    minIntervalSec: number;
    maxSuggestions: number;
    minPriority: AiAdvisorPriority | string;
    staleTimeoutSec: number;
    includeInstallerHints: boolean;
    categories: AiAdvisorCategories;
...
type AiAdvisorSuggestion = {
    id: string;
    category: string;
    severity: AiAdvisorSeverity | string;
    priority: number;
    icon?: string;
    title: string;
    text: string;
...
type AiAdvisorPlanState = {
    mode: AiAdvisorMode | string;
    items: AiAdvisorDailyPlanItem[];
    text: string;
};

type AiAdvisorLearningState = {
    anomaly: boolean;
    anomalyText: string;
    forecastQualityPct: number | null;
    peakLearningText: string;
    [key: string]: any;
};

type AiAdvisorRuntimeSample = {
    ts: number;
    hour?: number;
    gridImportW?: number;
    houseLoadW?: number;
...
type AiAdvisorSnapshot = {
    ...
    pvForecastUsable?: boolean;
    peakActive?: boolean;
    [key: string]: any;
};

type AiAdvisorConfigLike = AiAdvisorRuntimeConfig;
type AiAdvisorSnapshotLike = Partial<AiAdvisorSnapshot> & AiAdvisorUnknownRecord;

type AiAdvisorStorageSocInfo = {
    value: number | null;
    source: string;
    farmActive: boolean;
};
 ...
 * TypeScript: Parameter, Rückgabewert und verwendete Config-/State-Objekte später explizit typisieren.
 */
function priorityText(priorities: any, options: any = {}) {
...
class AiAdvisorModule extends BaseModule {
    /**
     * TypeScript-Migrationshinweis (DE): Klassenfelder des KI-Beraters.
     *
     * Zweck:
     * Die alte JS-Klasse legt viele Felder dynamisch im Konstruktor an. Für die
     * Migration werden die wichtigsten Felder hier explizit beschrieben, damit
     * spätere Änderungen an Lernzustand, Tagesfahrplan oder Veröffentlichungslogik
     * nachvollziehbar und typisierbar bleiben.
     */
    adapter: AiAdvisorAdapterLike;
    dp: AiAdvisorDpRegistryLike;
    _inited: boolean;
    _lastRunMs: number;
    _lastHash: string;
    _lastDisabledWriteMs: number;
    _samples: AiAdvisorRuntimeSample[];
    _peakHourBuckets: number[];
    _forecastChecks: AiAdvisorForecastCheck[];
    _forecastQualityPct: number | null;
    _forecastQualityText: string;
    _lastLearning: AiAdvisorLearningState;
    _lastPlan: AiAdvisorPlanState;
    [key: string]: any;
     ...
     */
    constructor(adapter: AiAdvisorAdapterLike, dpRegistry: AiAdvisorDpRegistryLike) {
     ...
     * TypeScript: Parameter, Rückgabewert und verwendete Config-/State-Objekte später explizit typisieren.
     */
    _cachedValue(id: string, fallback: any): any {
        try {
    ...
    _cfg(): AiAdvisorConfigLike {
        ...
        return {
            ...
            minPriority: (['info', 'warning', 'action', 'critical'].includes(minPriority) ? minPriority : 'info') as AiAdvisorPriority,
    ...
    async _ensureStates() {
         ...
         * TypeScript-Hinweis: Beim TypeScript-Umbau Parameter, Rückgabewert und verwendete State-/Config-Struktur explizit typisieren.
         */
        const mk = async (id: string, name: string, type: 'boolean' | 'number' | 'string', role: string, unit: string | undefined = undefined): Promise<void> => {
     ...
     * TypeScript: Parameter, Rückgabewert und verwendete Config-/State-Objekte später explizit typisieren.
     */
    async _readStorageSocPct(staleTimeoutSec: number = 300): Promise<AiAdvisorStorageSocInfo> {
         ...
         * TypeScript: Parameter, Rückgabewert und verwendete Config-/State-Objekte später explizit typisieren.
         */
        const readCandidate = async (entry: { key: string; type: string }): Promise<number | null> => {
         ...
         * TypeScript: Parameter, Rückgabewert und verwendete Config-/State-Objekte später explizit typisieren.
         */
        const readFirst = async (list: Array<{ key: string; type: string }>): Promise<{ value: number | null; source: string }> => {
     ...
     * TypeScript: Parameter, Rückgabewert und verwendete Config-/State-Objekte später explizit typisieren.
     */
    _updateLearning(snapshot: AiAdvisorSnapshotLike, cfg: AiAdvisorConfigLike): any {
        const s = Object.assign({}, snapshot || {}) as AiAdvisorSnapshotLike;
     ...
     * TypeScript: Parameter, Rückgabewert und verwendete Config-/State-Objekte später explizit typisieren.
     */
    _buildSuggestions(s: AiAdvisorSnapshotLike, cfg: AiAdvisorConfigLike): AiAdvisorSuggestion[] {
        const out = [];
        ...
        const categoryAllowed = (item) => {
            ...
            const cats: Record<string, boolean> = cfg.categories || {};
     ...
     * TypeScript: Parameter, Rückgabewert und verwendete Config-/State-Objekte später explizit typisieren.
     */
    _score(snapshot: AiAdvisorSnapshotLike, suggestions: AiAdvisorSuggestion[]): number {
        let score = 72;
     ...
     * TypeScript: Parameter, Rückgabewert und verwendete Config-/State-Objekte später explizit typisieren.
     */
    async _publishDisabled(cfg: AiAdvisorConfigLike, now: number): Promise<void> {
     ...
     * TypeScript: Parameter, Rückgabewert und verwendete Config-/State-Objekte später explizit typisieren.
     */
    async _publish(snapshot: AiAdvisorSnapshotLike, suggestions: AiAdvisorSuggestion[], cfg: AiAdvisorConfigLike): Promise<void> {
        const now = Date.now();
        ...
        const learning: any = this._lastLearning || {};
        const snap = {
            ts: now,
            enabled: true,
            advisoryOnly: true,
    ...
    async tick(): Promise<void> {
        ...
        this._lastRunMs = now;
        let snapshot: any = await this._snapshot();

// @ts-nocheck
/**
 * TypeScript-Parallelspiegel: ems/modules/threshold-control.js
 *
 * Zweck:
 * Diese Datei ist die TypeScript-Vorbereitung der bestehenden JavaScript-Runtime-Datei.
 * Sie wird noch nicht produktiv ausgeführt. Die produktive Quelle bleibt vorerst:
 * ems/modules/threshold-control.js
 *
 * Zusammenhang:
...
function clamp(v, minV, maxV, fallback = null) {
    ...
    if (n === null)
        return fallback;
    if (Number.isFinite(minV) && n < minV)
        return minV;
    if (Number.isFinite(maxV) && n > maxV)
        return maxV;
    return n;
}
/**
...
class ThresholdControlModule extends BaseModule {
    ...
    _normalizeCompare(raw) {
        ...
        if (s === 'below' || s === '<' || s === 'lt' || s === 'less' || s === 'kleiner')
            return 'below';
        return 'above';
    }
    /**
    ...
    _buildRulesFromConfig() {
        ...
        for (let i = 0; i < list.length; i++) {
            ...
            const estimatedPowerW = Math.max(0, clamp(r.estimatedPowerW ?? r.installedPowerW ?? r.maxPowerW, 0, 1e12, 0));
    ...
    _ruleHasExclusiveAuthority(r, owner) {
        if (String(owner || '').startsWith('manual.'))
            return true;
    ...
    async deactivate() {
        ...
        for (const r of this._rules || []) {
            ...
            try {
                ...
                if (!stopped.ok)
                    failures.push(`${r.id}:${stopped.reason || 'safe-stop-not-confirmed'}`);
            }
            catch (error) {
        ...
        if (failures.length)
            throw new Error(`threshold-safe-stop-failed:${failures.join(',')}`);
    ...
    async _forceRuleSafeOff(r, status = 'safe-stop') {
        if (!r || !r.outputId)
            return { ok: true, attempted: false, reason: 'no-output' };
        const dp = this.dp;
        if (!dp || typeof dp.upsert !== 'function')
            return { ok: false, attempted: false, reason: 'dp-registry-missing' };
    ...
    async _readRuleOutput(r) {
        if (!r || !r.outputId || !this.adapter || typeof this.adapter.getForeignStateAsync !== 'function')
            return null;
        try {
    ...
    async tick() {
        ...
        for (const r of this._rules) {
            ...
            if (this.dp && r.userCanToggle) {
                ...
                if (Number.isFinite(mv))
                    userMode = Math.max(0, Math.min(2, Math.round(mv)));
                else
                    userMode = userEnabled ? 1 : 0;
            }
            ...
            if (!mem.initialized && (onReadback === true || offReadback === true)) {
                mem.active = onReadback === true;
            ...
            if (r.safetyRelevant === true) {
                ...
                if (want && runtimeSafetyRequired && !(estimatedPowerW > 0)) {
                    ...
                    if (runtimeSafetyRequired) {
                        try {
                            liveSafetyEnvelope(this.adapter, this.dp, { now: Date.now() });
                        }
                        catch (error) {
            ...
            try {
                wrote = await this._writeRuleOutput(r, want, isManual);
            }
            catch (_e) {
                wrote = false;
            }
        ...
        if (safetyFailure) {
            try {
                this.adapter?._nwRequestImmediateEmsTick?.('safety:threshold-write', 100);
            }
            catch (_tickError) { }

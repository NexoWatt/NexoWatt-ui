 ...
 *
 * Original-Hash: 52b073a5de142221fd2670c41a90b144b0b467b0da5de145b3c76fa1c1fd5b1b
 */
 ...
 * Quell-Hash: sha256:7a35baf405a3989a89dbcd3c31ec1e4162fc11e50f2ad1e24db85e747262bcc3
...
(function () {
    ...
    const LIVE_CONTROL_VERSION = '0.8.183';
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
    const ruleBuilder = () => record(window.NexoWattOperatingStrategiesRuleBuilder);
/**
...
/**
 * Code-Teil: normalizeSystemMappings
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
    function normalizeSystemMappings(input) {
        const source = record(input);
        return {
 ...
 */
    function normalizeConfig(input) {
        ...
        return {
            ...
            rules: normalizedRules.map((entry) => ({
                ...entry,
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
    function eosAppActive(config, appId, enableFlag = '') {
        const app = record(record(record(config.emsApps).apps)[appId]);
        if (app.installed !== true || app.enabled !== true)
            return false;
        if (enableFlag && config[enableFlag] !== true)
            return false;
        return true;
    }
/**
 * Code-Teil: flowSlotFor
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
    function flowControlMappings(ctrlInput, includeStages = false) {
 ...
 */
    function deriveStorageFarmResources(config) {
        if (!eosAppActive(config, 'storagefarm', 'enableStorageFarm'))
            return [];
        ...
        return list(farm.storages).map((storage, index) => {
            ...
            if (storage.enabled !== true || (reads + writes) === 0)
                return null;
            return {
 ...
 */
    function deriveStorageResource(config) {
        if (!eosAppActive(config, 'storage', 'enableStorageControl'))
            return [];
 ...
 */
    function deriveEvcsResources(config) {
        if (!eosAppActive(config, 'charging') || config.enableChargingManagement !== true)
            return [];
        ...
        return list(settingsConfig.evcsList).map((row, index) => {
            ...
            if (row.enabled !== true || (reads + writes) === 0)
                return null;
 ...
 */
    function deriveFlowConsumerResources(config) {
        ...
        return list(flowSlots.consumers).map((slot, index) => {
            ...
            const readCandidates = [powerDp, ctrl.switchReadId, ctrl.setpointReadId, ctrl.sgReadyAReadId, ctrl.sgReadyBReadId, ctrl.sgReady1ReadId, ctrl.sgReady2ReadId];
            const writeCandidates = [ctrl.switchWriteId, ctrl.setpointWriteId, ctrl.sgReadyAWriteId, ctrl.sgReadyBWriteId, ctrl.sgReady1WriteId, ctrl.sgReady2WriteId];
            for (let stage = 1; stage <= 12; stage += 1) {
            ...
            const reads = readCandidates.filter(truthyDp).length;
            const writes = writeCandidates.filter(truthyDp).length;
            if (slot.enabled !== true || (reads + writes) === 0)
                return null;
            ...
            if ((consumerType === 'heatingRod' || /heizstab|heating.?rod/i.test(consumerType)) && !heatingActive)
                return null;
            if ((consumerType === 'heatPump' || consumerType === 'cooling' || inferredCooling) && !thermalActive)
                return null;
            // Dedicated Thermik-/Heizstabmodule expose their own device rows. Avoid duplicates.
            if ((thermalActive && (consumerType === 'heatPump' || consumerType === 'cooling' || inferredCooling)) || (heatingActive && consumerType === 'heatingRod'))
                return null;
            return {
 ...
 */
    function deriveModuleDevices(config, key, label, tab, resourceType) {
        ...
        if (!eosAppActive(config, appId, enableFlag))
            return [];
        ...
        return list(moduleConfig.devices).map((device, index) => {
            ...
            const writes = [device.switchWriteId, device.setpointWriteId, ctrl.switchWriteId, ctrl.setpointWriteId];
            if (key === 'thermal') {
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
    function applyRoleOverride(resourceInput, roleOverride) {
        const resource = { ...clone(resourceInput), roleOverride };
        if (roleOverride === 'cooling')
            return { ...resource, resourceType: 'thermal', resourceSubtype: 'cooling', controlType: resource.controlType === 'monitor' ? 'thermal' : resource.controlType };
        if (roleOverride === 'heatPump')
            return { ...resource, resourceType: 'thermal', resourceSubtype: 'heatPump', controlType: resource.controlType === 'monitor' ? 'thermal' : resource.controlType };
        if (roleOverride === 'heatingRod')
            return { ...resource, resourceType: 'thermal', resourceSubtype: 'heatingRod', controlType: resource.controlType === 'monitor' ? 'stepped' : resource.controlType };
        if (roleOverride === 'flexConsumer')
            return { ...resource, resourceType: 'consumer', resourceSubtype: 'flexible' };
        if (roleOverride === 'storage')
            return { ...resource, resourceType: 'storage', resourceSubtype: 'battery' };
        if (roleOverride === 'chargingPoint')
            return { ...resource, resourceType: 'chargingPoint', resourceSubtype: 'ev', controlType: resource.controlType === 'monitor' ? 'energyTarget' : resource.controlType };
        return resource;
    }
/**
 * Code-Teil: strategyResourceCatalog
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
    function strategyResourceCatalog() {
...
/**
 * Code-Teil: existingReadMappingInput
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
    function existingReadMappingInput(resource, link, key, label) {
...
/**
 * Code-Teil: existingReadMappingsHtml
 *
 * Zweck:
 * Automatisch markierter Funktion-Abschnitt aus der ursprünglichen JavaScript-Datei.
 * Dieser Kommentar dient als Orientierung für die schrittweise TypeScript-Migration.
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
    function existingReadMappingsHtml(resource, link) {
        const fields = [
        ...
        if (resource.resourceType === 'thermal')
            fields.push(['temperatureReadId', 'Temperatur']);
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
    function profilesHtml(profiles, resources) {
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
            const find = (selector, attr) => Array.from(document.querySelectorAll(selector)).find((entry) => text(entry.getAttribute(attr)) === sourceId);
            const priorityInput = find('[data-os-link-priority]', 'data-os-link-priority');
            const roleInput = find('[data-os-link-role]', 'data-os-link-role');
            const modeInput = find('[data-os-link-control-mode]', 'data-os-link-control-mode');
            const commissioningInput = find('[data-os-link-commissioning]', 'data-os-link-commissioning');
            const autoSourceInput = find('[data-os-link-auto-source]', 'data-os-link-auto-source');
            const fallbackInput = find('[data-os-link-fallback]', 'data-os-link-fallback');
            const staleInput = find('[data-os-link-stale]', 'data-os-link-stale');
            const previous = record(existingMap.get(sourceId));
            const mappings = normalizeMappings(previous.mappings);
            document.querySelectorAll('[data-os-link-map-source]').forEach((node) => {
                if (text(node.getAttribute('data-os-link-map-source')) !== sourceId)
                    return;
                const key = text(node.getAttribute('data-os-link-map-key'));
                if (key)
                    mappings[key] = text(node.value);
            });
        ...
        list(workingConfig.resourceLinks).forEach((entry) => {
            ...
            if (!sourceId || renderedIds.has(sourceId))
                return;
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
    function syncGlobalFromDom() {
        const mode = byId('osMode');
        const stage = byId('osAutoStage');
        const commissioning = byId('osCommissioningConfirmed');
        const takeover = byId('osControlTakeoverEnabled');
        const execution = byId('osWriteExecutionEnabled');
        const ttl = byId('osRequestTtlSeconds');
        const fallback = byId('osGlobalFallback');
        if (mode)
            workingConfig.mode = mode.value === 'active' ? 'active' : 'observe';
        if (commissioning)
            workingConfig.commissioningConfirmed = commissioning.checked;
        if (takeover)
            workingConfig.controlTakeoverEnabled = takeover.checked;
        if (execution)
            workingConfig.writeExecutionEnabled = execution.checked;
        workingConfig.autoControl = record(workingConfig.autoControl);
        if (stage)
            workingConfig.autoControl.stage = ['shadow', 'commissioning', 'active'].includes(stage.value) ? stage.value : 'shadow';
        if (ttl)
            workingConfig.autoControl.requestTtlSeconds = integer(ttl.value, 15, 3, 60);
        if (fallback)
            workingConfig.autoControl.fallback = fallback.value === 'pause' ? 'pause' : 'standardAuto';
    }
/**
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
        const getCheck = (id, fallback = false) => byId(id)?.checked ?? fallback;
/**
 * Code-Teil: getValue
 *
 * Zweck:
 ...
 * welcher konkrete Code-Abschnitt später typisiert, getestet und übernommen werden muss.
 */
        const getValue = (id, fallback = '') => text(byId(id)?.value, fallback);
        ...
        document.querySelectorAll('[data-os-system-map]').forEach((node) => {
            const key = text(node.getAttribute('data-os-system-map'));
            if (key)
                systemMappings[key] = text(node.value);
        });
        ...
        syncProfilesFromDom();
        const builder = ruleBuilder();
        if (typeof builder.syncFromDom === 'function')
            workingConfig = builder.syncFromDom(workingConfig, strategyResourceCatalog());
 ...
 */
    async function render(mount, config = {}, enabled = false) {
        ...
        const builder = ruleBuilder();
        if (typeof builder.resetSimulationResult === 'function')
            builder.resetSimulationResult();
        rerender();
    }
/**
 * Code-Teil: apply

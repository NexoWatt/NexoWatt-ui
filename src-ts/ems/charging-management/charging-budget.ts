 ...
 */
function applyMinCap(current: number | null, cap: number | null): { value: number | null; applied: boolean } {
  if (cap === null || !Number.isFinite(cap) || cap < 0) return { value: current, applied: false };
  if (current === null || !Number.isFinite(current)) return { value: Math.max(0, cap), applied: true };
 ...
 */
function appendBudgetModeSuffix(mode: string, suffix: string): string {
 ...
 */
export function computeChargingBudgetSafetyCaps(input: ChargingBudgetSafetyCapInput): ChargingBudgetSafetyCapResult {
  ...
  let finalStatus = 'ok';
  if (gridApplied && phaseApplied) finalStatus = 'limited_grid_import_and_phase';
  else if (gridApplied) finalStatus = 'limited_grid_import';
  else if (phaseApplied) finalStatus = 'limited_phase_cap';
  if (p14aApplied && finalStatus === 'ok') finalStatus = 'limited_14a';
 ...
 */
export function compareChargingBudgetSafetyCaps(js: Record<string, unknown>, input: ChargingBudgetSafetyCapInput): ChargingBudgetShadowComparison {
  const ts = computeChargingBudgetSafetyCaps(input);
  const mismatches: Array<{ field: string; js: unknown; ts: unknown }> = [];
  const cmp = (field: string, jsVal: unknown, tsVal: unknown): void => {
    if (jsVal !== tsVal) mismatches.push({ field, js: jsVal, ts: tsVal });
  };
 ...
 */
export function buildChargingBudgetSafetyCapsProductive(js: Record<string, unknown>, input: ChargingBudgetSafetyCapInput): ChargingBudgetProductiveDecision {
 ...
 */
export function buildChargingBudgetProductiveDecision(js: Record<string, unknown>, input: ChargingBudgetSafetyCapInput): ChargingBudgetProductiveDecisionFlat {
